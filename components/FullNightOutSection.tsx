...
import { computeFullNightOutScore } from '@/lib/fullNightOutScore';

interface FullNightOutSectionProps {
  buffet: any;
}

export default function FullNightOutSection({ buffet }: FullNightOutSectionProps) {
  const result = computeFullNightOutScore(buffet);

  if (result.fullNightOutScore < 25) return null;

  const { fullNightOutScore, fullNightOutTier, fullNightOutTierEmoji, subScores, positiveSignals, negativeSignals } = result;

  const summary = `Score ${fullNightOutScore}/100 · ${positiveSignals.length} positive signal${positiveSignals.length !== 1 ? 's' : ''}`;

  const SUB_SCORE_LABELS: Record<keyof typeof subScores, { label: string; max: number }> = {
    lateNightDining:        { label: 'Late-night hours',   max: 30 },
    drinksOnPremises:       { label: 'Drinks on premises', max: 20 },
    nightlifeSurroundings:  { label: 'Nightlife nearby',   max: 25 },
    entertainmentAndEnergy: { label: 'Entertainment',      max: 15 },
    weekendPotential:       { label: 'Weekend potential',  max: 10 },
  };
    ...
    <DisclosureCard
      title={
        <span className="flex items-center gap-2">
          <span aria-hidden>{fullNightOutTierEmoji}</span>
          <span>{fullNightOutTier}</span>
        </span>
      }
      summary={summary}
      defaultOpen
    ...
    >
      <div className="space-y-5">
        ...
        <p className="text-xs text-[var(--muted)]">
          Scored from closing hours, on-premises drinks, nearby nightlife, entertainment, and weekend hours.
        </p>
      </div>
    </DisclosureCard>
  );

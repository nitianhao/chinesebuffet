/**
 * Generate SEO-optimized "Financial Services" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.financialServices (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-financial-services.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-financial-services.ts --concurrency 5
 *   npx tsx scripts/generate-poi-financial-services.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-financial-services.ts --resume
 *   npx tsx scripts/generate-poi-financial-services.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Financial Services';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'financial-services.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Banking
    'bank': { bucketKey: 'banking', bucketLabel: 'Banking', typePhrase: 'banking services' },
    'atm': { bucketKey: 'banking', bucketLabel: 'Banking', typePhrase: 'banking services' },
    
    // Insurance
    'insurance': { bucketKey: 'insurance', bucketLabel: 'Insurance', typePhrase: 'insurance services' },
    'insurance_agent': { bucketKey: 'insurance', bucketLabel: 'Insurance', typePhrase: 'insurance services' },
    
    // Financial planning & investment
    'financial': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'financial_advice': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'financial_services': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'investment': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'stock_broker': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'mortgage': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'mortgage_lender': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    'credit_card_company': { bucketKey: 'investment', bucketLabel: 'Financial planning & investment', typePhrase: 'financial planning and investment services' },
    
    // Money services
    'money_lender': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'money_transfer': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'bureau_de_change': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'check_cashing': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'cheque_cashing': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'payment_terminal': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    'payment_centre': { bucketKey: 'money', bucketLabel: 'Money services', typePhrase: 'money services' },
    
    // Pawn & gold
    'pawnbroker': { bucketKey: 'pawn', bucketLabel: 'Pawn & gold', typePhrase: 'pawn and gold services' },
    'gold_buyer': { bucketKey: 'pawn', bucketLabel: 'Pawn & gold', typePhrase: 'pawn and gold services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other financial services', typePhrase: 'financial services' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'financial_advice': 'Financial advisor',
    ...
    'check_cashing': 'Check cashing service',
    'cheque_cashing': 'Check cashing service',
    'payment_terminal': 'Payment terminal',
    'payment_centre': 'Payment center',
    'mortgage': 'Mortgage service',
  ...
  return fallbacks[cat] || 'Financial service';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Financial Services
...
/**
 * Update names for bucket context (no special handling needed for Financial Services)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Financial Services
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'financial services';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'financial services';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 financial service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} financial services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Financial Services
 ...
 */
function validateOutput(output: FinancialServicesOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: FinancialServicesOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if financialServices already exists (unless force)
  if (!options.force && buffet.financialServices && buffet.financialServices.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: FinancialServicesOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ financialServices: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: FinancialServicesOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (financialServices already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Financial Services Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.financialServices`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has financialServices (unless force)
        if (!force && buffet.financialServices && buffet.financialServices.trim().length > 0) {

// Script to extract "Atmosphere" from additionalInfo field in database
// and populate the atmosphere field
// Run with: node scripts/extract-atmosphere-from-db.js [--all]
...
/**
 * Extract "Atmosphere" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of atmosphere items, or null if not found
 */
function extractAtmosphere(additionalInfo) {
  if (!additionalInfo) {
    return null;
  }
  ...
  // Check if "Atmosphere" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const atmosphereKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'atmosphere' || key === 'Atmosphere'
  );
  ...
  const atmosphereArray = infoObj[atmosphereKey];

  if (!Array.isArray(atmosphereArray) || atmosphereArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "Casual": true }, { "Cozy": true }]
  const atmosphereItems = [];
  
  for (const item of atmosphereArray) {
    if (typeof item === 'object' && item !== null) {
      ...
      for (const key of keys) {
        if (item[key] === true || item[key]) {
          atmosphereItems.push(key);
        }
      }
    } else if (typeof item === 'string') {
      // If it's already a string, just add it
      atmosphereItems.push(item);
    }
  }

  if (atmosphereItems.length === 0) {
    return null;
  }

  // Return as comma-separated string
  return atmosphereItems.join(', ');
}

async function extractAtmosphereFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Atmosphere" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Atmosphere"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithAtmosphere = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const atmosphere = extractAtmosphere(additionalInfo);
        ...
        const currentAtmosphere = buffet.atmosphere || null;
    ...
    console.log(`\n✓ Found ${foundWithAtmosphere} records with "Atmosphere" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct atmosphere value or have no "Atmosphere" data.\n');
      return;
    }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → atmosphere: ${update.oldValue || 'null'} → "${update.atmosphere}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Atmosphere": ${foundWithAtmosphere}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-atmosphere-from-db.js --all\n');
    }
...
extractAtmosphereFromDB().catch(error => {

/**
 * Generate SEO-optimized "Industrial & Manufacturing" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.industrialManufacturing (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-industrial-manufacturing.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-industrial-manufacturing.ts --concurrency 5
 *   npx tsx scripts/generate-poi-industrial-manufacturing.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-industrial-manufacturing.ts --resume
 *   npx tsx scripts/generate-poi-industrial-manufacturing.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Industrial & Manufacturing';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'industrial-manufacturing.checkpoint.json');
...
type BuffetRecord = {
  ...
  name?: string | null;
  placeId?: string | null;
  industrialManufacturing?: string | null;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other industrial and manufacturing', typePhrase: 'industrial and manufacturing' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Industrial & Manufacturing
...
/**
 * Update names for bucket context (no special handling needed for Industrial & Manufacturing)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Industrial & Manufacturing
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'industrial and manufacturing';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'industrial and manufacturing';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 industrial and manufacturing service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} industrial and manufacturing services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Industrial & Manufacturing
...
async function processBuffet(
  ...
  // Skip if industrialManufacturing already exists (unless force)
  if (!options.force && buffet.industrialManufacturing && buffet.industrialManufacturing.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ industrialManufacturing: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (industrialManufacturing already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Industrial & Manufacturing Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.industrialManufacturing`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has industrialManufacturing (unless force)
        if (!force && buffet.industrialManufacturing && buffet.industrialManufacturing.trim().length > 0) {

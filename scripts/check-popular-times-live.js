// Script to check popularTimesLivePercent and popularTimesLiveText fields
...
const fieldsToCheck = ['popularTimesLivePercent', 'popularTimesLiveText'];
...
async function checkFields() {
  ...
  try {
      ...
      fieldsToCheck.forEach(field => {
        if (field in buffet) {
          ...
          if (buffet[field] !== null && buffet[field] !== undefined) {
            ...
            if (field === 'popularTimesLivePercent') {
              // For popularTimesLivePercent, it's a number
            ...
            } else if (field === 'popularTimesLiveText') {
              // For popularTimesLiveText, it's a string
    ...
    fieldsToCheck.forEach(field => {
      ...
      if (field === 'popularTimesLivePercent' && stats.minValue !== null) {
        ...
        console.log(`Unique text values: ${stats.uniqueValues.size}`);
        if (stats.uniqueValues.size > 0 && stats.uniqueValues.size <= 20) {
          console.log(`Unique text list: ${Array.from(stats.uniqueValues).join(', ')}`);
        } else if (stats.uniqueValues.size > 20) {
          console.log(`Unique text list (first 20): ${Array.from(stats.uniqueValues).slice(0, 20).join(', ')}...`);
        }
      }
      ...
      if (stats.examples.length > 0) {
        ...
        stats.examples.forEach((example, index) => {
          ...
          if (field === 'popularTimesLivePercent') {
            console.log(`    popularTimesLivePercent: ${example.value} (type: ${typeof example.value})`);
          } else {
            console.log(`    popularTimesLiveText: "${example.value}"`);
          }
        });
      } else {

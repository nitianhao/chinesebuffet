// Script to migrate noiseLevel from buffets table to structuredData table
// Run with: node scripts/migrate-noise-level.js [--all]
...
async function migrateNoiseLevel() {
  try {
    console.log('================================================================================');
    console.log('Migrating noiseLevel to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have noiseLevel set
    const buffets = allBuffets.buffets.filter(b => b.noiseLevel !== null && b.noiseLevel !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with noiseLevel`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with noiseLevel field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.noiseLevel),
              type: 'noiseLevel',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: noiseLevel)`);
        console.log(`  → Value: ${buffet.noiseLevel}\n`);

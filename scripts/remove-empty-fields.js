// Script to remove empty fields (googleFoodUrl, fid, hotelAds, cid) from all buffets in the database
...
// Fields to remove
const fieldsToRemove = ['googleFoodUrl', 'fid', 'hotelAds', 'cid'];

async function removeEmptyFields() {
  ...
  try {
    ...
    console.log(`Found ${buffetsToUpdate.length} buffets that need updating\n`);

    if (buffetsToUpdate.length === 0) {
      console.log('✓ No buffets need updating. All empty fields are already removed!');
      return;
    }
    ...
    console.log(`\n✓ Successfully removed empty fields from ${updatedCount} buffets!`);

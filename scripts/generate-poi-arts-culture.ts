/**
 * Generate SEO-optimized "Arts & Culture" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.artsCulture (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-arts-culture.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-arts-culture.ts --concurrency 5
 *   npx tsx scripts/generate-poi-arts-culture.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-arts-culture.ts --resume
 *   npx tsx scripts/generate-poi-arts-culture.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Arts & Culture';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'arts-culture.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Museums & galleries
    'museum': { bucketKey: 'museums', bucketLabel: 'Museums & galleries', typePhrase: 'museums' },
    'gallery': { bucketKey: 'museums', bucketLabel: 'Museums & galleries', typePhrase: 'museums' },
    'art': { bucketKey: 'museums', bucketLabel: 'Museums & galleries', typePhrase: 'museums' },
    'artwork': { bucketKey: 'museums', bucketLabel: 'Museums & galleries', typePhrase: 'museums' },
    'arts_centre': { bucketKey: 'museums', bucketLabel: 'Museums & galleries', typePhrase: 'museums' },
    
    // Libraries & archives
    'library': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries' },
    'archive': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries' },
    'archives': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries' },
    
    // Theaters & performance venues
    'theatre': { bucketKey: 'theaters', bucketLabel: 'Theaters & performance venues', typePhrase: 'theaters' },
    'theater': { bucketKey: 'theaters', bucketLabel: 'Theaters & performance venues', typePhrase: 'theaters' },
    'cinema': { bucketKey: 'theaters', bucketLabel: 'Theaters & performance venues', typePhrase: 'theaters' },
    'concert_hall': { bucketKey: 'theaters', bucketLabel: 'Theaters & performance venues', typePhrase: 'theaters' },
    'music_venue': { bucketKey: 'theaters', bucketLabel: 'Theaters & performance venues', typePhrase: 'theaters' },
    
    // Planetariums & observatories
    'planetarium': { bucketKey: 'planetariums', bucketLabel: 'Planetariums & observatories', typePhrase: 'planetariums' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other arts & culture', typePhrase: 'arts & culture venues' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  return fallbacks[cat] || 'Arts & culture venue';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Arts & Culture
...
/**
 * Update names for bucket context (no special handling needed for Arts & Culture)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Arts & Culture
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'arts & culture venues';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'arts & culture venues';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 arts & culture venue is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} arts & culture venues are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Arts & Culture (removed social-service-specific hooks)
 ...
 */
function validateOutput(output: ArtsCultureOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: ArtsCultureOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if artsCulture already exists (unless force)
  if (!options.force && buffet.artsCulture && buffet.artsCulture.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: ArtsCultureOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ artsCulture: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (artsCulture already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Arts & Culture Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.artsCulture`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has artsCulture (unless force)
        if (!force && buffet.artsCulture && buffet.artsCulture.trim().length > 0) {

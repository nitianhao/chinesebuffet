/**
 * Generate SEO-optimized "Religious & Spiritual" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.religiousSpiritual (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-religious-spiritual.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-religious-spiritual.ts --concurrency 5
 *   npx tsx scripts/generate-poi-religious-spiritual.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-religious-spiritual.ts --resume
 *   npx tsx scripts/generate-poi-religious-spiritual.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Religious & Spiritual';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'religious-spiritual.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Places of worship
    'place_of_worship': { bucketKey: 'worship', bucketLabel: 'Places of worship', typePhrase: 'places of worship' },
    'church': { bucketKey: 'worship', bucketLabel: 'Places of worship', typePhrase: 'places of worship' },
    'monastery': { bucketKey: 'worship', bucketLabel: 'Places of worship', typePhrase: 'places of worship' },
    'religion': { bucketKey: 'worship', bucketLabel: 'Places of worship', typePhrase: 'places of worship' },
    
    // Cemeteries & memorials
    'grave_yard': { bucketKey: 'cemeteries', bucketLabel: 'Cemeteries & memorials', typePhrase: 'cemeteries and memorials' },
    'grave': { bucketKey: 'cemeteries', bucketLabel: 'Cemeteries & memorials', typePhrase: 'cemeteries and memorials' },
    'crematorium': { bucketKey: 'cemeteries', bucketLabel: 'Cemeteries & memorials', typePhrase: 'cemeteries and memorials' },
    
    // Funeral services
    'funeral_directors': { bucketKey: 'funeral', bucketLabel: 'Funeral services', typePhrase: 'funeral services' },
    'funeral_hall': { bucketKey: 'funeral', bucketLabel: 'Funeral services', typePhrase: 'funeral services' },
    
    // Other
    'peace_pole': { bucketKey: 'other', bucketLabel: 'Other religious & spiritual sites', typePhrase: 'religious and spiritual sites' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other religious & spiritual sites', typePhrase: 'religious & spiritual sites' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'peace_pole': 'Peace pole',
  };
  
  return fallbacks[cat] || 'Religious & spiritual site';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Religious & Spiritual
...
/**
 * Update names for bucket context (no special handling needed for Religious & Spiritual)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Religious & Spiritual
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'religious & spiritual sites';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'religious & spiritual sites';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  const venueWord = poiCount === 1 ? 'site' : 'sites';
  // Single-item: remove "about" hedge
  if (poiCount === 1) {
    const sentence1 = `1 religious & spiritual site is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} religious & spiritual sites are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Religious & Spiritual
 ...
 */
function validateOutput(output: ReligiousSpiritualOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: ReligiousSpiritualOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if religiousSpiritual already exists (unless force)
  if (!options.force && buffet.religiousSpiritual && buffet.religiousSpiritual.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: ReligiousSpiritualOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ religiousSpiritual: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: ReligiousSpiritualOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (religiousSpiritual already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Religious & Spiritual Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.religiousSpiritual`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has religiousSpiritual (unless force)
        if (!force && buffet.religiousSpiritual && buffet.religiousSpiritual.trim().length > 0) {

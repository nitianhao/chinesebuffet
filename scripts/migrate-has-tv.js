// Script to migrate hasTv from buffets table to structuredData table
// Run with: node scripts/migrate-has-tv.js [--all]
...
async function migrateHasTv() {
  try {
    console.log('================================================================================');
    console.log('Migrating hasTv to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have hasTv set
    const buffets = allBuffets.buffets.filter(b => b.hasTv !== null && b.hasTv !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with hasTv`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with hasTv field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.hasTv),
              type: 'hasTv',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: hasTv)`);
        console.log(`  → Value: ${buffet.hasTv}\n`);

// Script to extract "Offerings" from additionalInfo field in database
// and populate the offerings field
// Run with: node scripts/extract-offerings-from-db.js [--all]
...
/**
 * Extract "Offerings" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of offerings, or null if not found
 */
function extractOfferings(additionalInfo) {
  if (!additionalInfo) {
    return null;
  }
  ...
  // Check if "Offerings" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const offeringsKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'offerings' || key === 'Offerings'
  );
  ...
  const offeringsArray = infoObj[offeringsKey];

  if (!Array.isArray(offeringsArray) || offeringsArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "All you can eat": true }, { "Vegan options": true }]
  const offeringsItems = [];
  
  for (const item of offeringsArray) {
    if (typeof item === 'object' && item !== null) {
      ...
      for (const key of keys) {
        if (item[key] === true || item[key]) {
          offeringsItems.push(key);
        }
      }
    } else if (typeof item === 'string') {
      // If it's already a string, just add it
      offeringsItems.push(item);
    }
  }
  ...
  return offeringsItems.join(', ');
}

async function extractOfferingsFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Offerings" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Offerings"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithOfferings = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const offerings = extractOfferings(additionalInfo);

        if (!offerings || offerings.trim() === '') {
          skipped++;
          continue;
        }
        ...
        const currentOfferings = buffet.offerings || null;
        if (currentOfferings === offerings) {
    ...
    console.log(`\n✓ Found ${foundWithOfferings} records with "Offerings" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct offerings value or have no "Offerings" data.\n');
      return;
    }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → offerings: ${update.oldValue || 'null'} → "${update.offerings}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Offerings": ${foundWithOfferings}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-offerings-from-db.js --all\n');
    }
...
extractOfferingsFromDB().catch(error => {

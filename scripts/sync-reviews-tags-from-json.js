// Script to sync reviewsTags data from apify-big-cities.json to instantDB buffet table
// Overrides existing values if already filled out
// Run with: node scripts/sync-reviews-tags-from-json.js [--all]
...
async function syncReviewsTags() {
  console.log('='.repeat(80));
  console.log('Syncing reviewsTags data from JSON to instantDB buffet table');
  ...
  try {
    ...
    // Filter records that have reviewsTags data
    const recordsWithTags = jsonData.filter(record => {
      const tags = record.reviewsTags;
      return tags && 
             tags !== 'null' &&
             tags !== null &&
    ...
    console.log(`✓ Found ${recordsWithTags.length} records with reviewsTags data\n`);

    if (recordsWithTags.length === 0) {
      console.log('No records with reviewsTags data found. Exiting.');
      return;
    }
    ...
    const recordsToProcess = migrateAll 
      ? recordsWithTags 
      : recordsWithTags.slice(0, testLimit);

    console.log(`Processing ${recordsToProcess.length} records...\n`);

    // Create a map of PlaceID -> reviewsTags (as JSON string) for quick lookup
    const tagsMap = new Map();
    for (const record of recordsToProcess) {
      if (record.PlaceID && record.reviewsTags) {
        let tagsValue;
        
        // If it's already a string, try to parse it to validate, then use it
        if (typeof record.reviewsTags === 'string') {
          try {
            // Try to parse to validate it's valid JSON
            JSON.parse(record.reviewsTags);
            tagsValue = record.reviewsTags.trim();
          } catch (e) {
            // If parsing fails, skip this record
            console.warn(`⚠ Skipping invalid JSON string for PlaceID ${record.PlaceID}`);
            continue;
          }
        } else if (typeof record.reviewsTags === 'object') {
          // Convert object/array to JSON string
          tagsValue = JSON.stringify(record.reviewsTags);
        } else {
          continue;
        }
        
        tagsMap.set(record.PlaceID, tagsValue);
      }
    }

    console.log(`✓ Created lookup map with ${tagsMap.size} PlaceIDs\n`);

    // Fetch buffets that match these PlaceIDs
    console.log('Fetching matching buffets from database...');
    const placeIds = Array.from(tagsMap.keys());
    ...
    for (const buffet of allBuffets) {
      try {
        ...
        const newTags = tagsMap.get(placeId);
        ...
        const oldValue = buffet.reviewsTags || null;
        updates.push({
          buffetId: buffet.id,
      ...
      try {
        const transactions = batch.map(update => 
          db.tx.buffets[update.buffetId].update({
            reviewsTags: update.reviewsTags
          })
        );
        ...
        for (const update of batch) {
          ...
          if (update.oldValue) {
            try {
              ...
              if (Array.isArray(parsed)) {
                oldValuePreview = `[array with ${parsed.length} tags]`;
              } else {
          ...
          try {
            const parsed = typeof update.reviewsTags === 'string' ? JSON.parse(update.reviewsTags) : update.reviewsTags;
            if (Array.isArray(parsed)) {
              const firstTags = parsed.slice(0, 3).map(t => t.title || t).join(', ');
              newValuePreview = `[${firstTags}${parsed.length > 3 ? ', ...' : ''}] (${parsed.length} tags)`;
            } else {
              newValuePreview = update.reviewsTags.substring(0, 60) + '...';
            }
          } catch (e) {
            newValuePreview = update.reviewsTags.substring(0, 60) + '...';
          }
          ...
          console.log(`  → reviewsTags: ${oldValuePreview} → ${newValuePreview}`);
          updated++;
        }
        ...
        for (const update of batch) {
          try {
            ...
            try {
              const parsed = typeof update.reviewsTags === 'string' ? JSON.parse(update.reviewsTags) : update.reviewsTags;
              if (Array.isArray(parsed)) {
                newValuePreview = `[${parsed.length} tags]`;
              } else {
                newValuePreview = update.reviewsTags.substring(0, 40) + '...';
              }
            } catch (e) {
              newValuePreview = update.reviewsTags.substring(0, 40) + '...';
            }
    ...
    if (!migrateAll && updated > 0) {
      ...
      console.log('  node scripts/sync-reviews-tags-from-json.js --all\n');
    }

// Script to migrate popularFor from buffets table to structuredData table
// Run with: node scripts/migrate-popular-for-to-structured-data.js [--all]
...
async function migratePopularFor() {
  console.log('='.repeat(80));
  console.log('Migrating popularFor to structuredData table');
  console.log('='.repeat(80));
  console.log(`Mode: ${migrateAll ? 'ALL records' : 'TEST (1 record only)'}\n`);

  try {
    // Query buffets - we'll filter for those with popularFor in JavaScript
    console.log('Querying buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    // Fetch buffets in batches and filter for those with popularFor
    while (true) {
      const query = {
        buffets: {
          $: {
      ...
      // Filter for buffets that have popularFor
    ...
    if (buffets.length === 0) {
      console.log('✓ No buffets found with popularFor');
      return;
    }

    console.log(`✓ Found ${buffets.length} buffet(s) with popularFor\n`);
    ...
    for (let i = 0; i < buffets.length; i++) {
      ...
      try {
        // Check if structuredData already exists for this buffet with type 'popularFor'
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {}
        ...
        const hasPopularFor = existingStructuredData.some(sd => sd.type === 'popularFor');
        ...
        // Parse the popularFor field
        // Note: popularFor is described as "What the restaurant is popular for" in the schema
        let popularForData = null;
        if (buffet.popularFor) {
          try {
            if (typeof buffet.popularFor === 'string') {
              // First, try to parse as JSON
              try {
                popularForData = JSON.parse(buffet.popularFor);
              } catch (jsonError) {
                // If not valid JSON, check if it's comma-separated
                if (buffet.popularFor.includes(',')) {
                  // Split by comma, trim each item, and create an array
                  popularForData = buffet.popularFor
                    .split(',')
                    .map(item => item.trim())
                    .filter(item => item.length > 0);
                } else {
                  // Single value, create array with one item
                  popularForData = [buffet.popularFor.trim()];
                }
              }
            } else if (Array.isArray(buffet.popularFor)) {
              // Already an array
              popularForData = buffet.popularFor;
            } else {
              // Already an object
            ...
            console.error(`⚠ Error parsing popularFor for buffet "${buffet.name}":`, parseError.message);
            errors++;
            continue;
          }
        }
        ...
        let dataString;
        if (Array.isArray(popularForData)) {
          dataString = JSON.stringify(popularForData);
        } else if (typeof popularForData === 'object') {
          dataString = JSON.stringify(popularForData);
        } else {
        ...
        if (migrateAll) {
          ...
          console.log(`  → Created structuredData record (id: ${structuredDataId}, type: popularFor)`);
        ...
        if (!migrateAll && migrated === 1) {
          ...
          console.log('  node scripts/migrate-popular-for-to-structured-data.js --all\n');
          break;
        }

// Script to migrate highlights from buffets table to structuredData table
// Run with: node scripts/migrate-highlights-to-structured-data.js [--all]
...
async function migrateHighlights() {
  ...
  console.log('Migrating highlights to structuredData table');
  console.log('='.repeat(80));
  console.log(`Mode: ${migrateAll ? 'ALL records' : 'TEST (1 record only)'}\n`);

  try {
    // Query buffets - we'll filter for those with highlights in JavaScript
    console.log('Querying buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    // Fetch buffets in batches and filter for those with highlights
    while (true) {
      const query = {
        buffets: {
          $: {
      ...
      // Filter for buffets that have highlights
    ...
    if (buffets.length === 0) {
      console.log('✓ No buffets found with highlights');
      return;
    }

    console.log(`✓ Found ${buffets.length} buffet(s) with highlights\n`);
    ...
    for (let i = 0; i < buffets.length; i++) {
      ...
      try {
        // Check if structuredData already exists for this buffet with type 'highlights'
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {}
        ...
        const hasHighlights = existingStructuredData.some(sd => sd.type === 'highlights');
        ...
        if (buffet.highlights) {
          try {
            if (typeof buffet.highlights === 'string') {
              // First, try to parse as JSON
              try {
                highlightsData = JSON.parse(buffet.highlights);
              } catch (jsonError) {
                // If not valid JSON, check if it's comma-separated
                if (buffet.highlights.includes(',')) {
                  // Split by comma, trim each item, and create an array
                  highlightsData = buffet.highlights
                    .split(',')
                    .map(item => item.trim())
                    .filter(item => item.length > 0);
                } else {
                  // Single value, create array with one item
                  highlightsData = [buffet.highlights.trim()];
                }
              }
            } else if (Array.isArray(buffet.highlights)) {
              // Already an array
              highlightsData = buffet.highlights;
            } else {
              // Already an object
            ...
            console.error(`⚠ Error parsing highlights for buffet "${buffet.name}":`, parseError.message);
            errors++;
            continue;
          }
        }
        ...
        if (Array.isArray(highlightsData)) {
          dataString = JSON.stringify(highlightsData);
        } else if (typeof highlightsData === 'object') {
          dataString = JSON.stringify(highlightsData);
        } else {
        ...
        if (migrateAll) {
          ...
          console.log(`  → Created structuredData record (id: ${structuredDataId}, type: highlights)`);
        ...
        if (!migrateAll && migrated === 1) {
          ...
          console.log('  node scripts/migrate-highlights-to-structured-data.js --all\n');
          break;
        }

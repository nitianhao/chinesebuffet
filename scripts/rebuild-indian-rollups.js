...
/**
 * Rebuild directory rollups for INDIAN buffet hub and detail pages.
 * Filters buffets by cuisineType='indian' and saves with 'indian:' prefixed types.
 * 
 * Usage:
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    try {
      const result = await db.query({
        buffets: {
          $: {
            where: { stateAbbr, cuisineType: 'indian' },
            limit: BATCH_SIZE,
            offset: offset,
          }
        }
...
async function main() {
  ...
  console.log('');
  console.log('🚀 Rebuilding INDIAN directory rollups...');
  console.log('');
  ...
  try {
    ...
    if (buildStates) {
      ...
      await saveRollup('indian:states', null, states);
      console.log('');
    }
    ...
    if (buildCities) {
      ...
      await saveRollup('indian:cities', null, cities);
      console.log('');
    }
    ...
    if (buildNeighborhoods) {
      ...
      for (const rollup of neighborhoodRollups) {
        try {
          await saveRollup('indian:cityNeighborhoods', rollup.key, rollup.data);
          saved++;
        } catch (error) {
    ...
    if (buildStateCities) {
      ...
      for (const rollup of stateCitiesRollups) {
        try {
          await saveRollup('indian:stateCities', rollup.key, rollup.data);
          saved++;
        } catch (error) {
    ...
    if (buildCityBuffets) {
      ...
      for (const rollup of cityBuffetsRollups) {
        try {
          await saveRollup('indian:cityBuffets', rollup.key, rollup.data);
          saved++;
        } catch (error) {
    ...
    if (buildNeighborhoodBuffets) {
      ...
      for (const rollup of neighborhoodBuffetsRollups) {
        try {
          await saveRollup('indian:neighborhoodBuffets', rollup.key, rollup.data);
          saved++;
        } catch (error) {
    ...
    if (buildCityFacets) {
      ...
      for (const rollup of cityFacetsRollups) {
        try {
          await saveRollup('indian:cityFacets', rollup.key, rollup.data);
          saved++;
        } catch (error) {

/**
 * Generate SEO-optimized "Communications & Technology" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.communicationsTechnology (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-communications-technology.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-communications-technology.ts --concurrency 5
 *   npx tsx scripts/generate-poi-communications-technology.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-communications-technology.ts --resume
 *   npx tsx scripts/generate-poi-communications-technology.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Communications & Technology';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'communications-technology.checkpoint.json');
...
type BuffetRecord = {
  ...
  name?: string | null;
  placeId?: string | null;
  communicationsTechnology?: string | null;
...
type UtilitiesInfrastructureOutput = {
  summary: string;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Fuel & charging
    'fuel': { bucketKey: 'fuel', bucketLabel: 'Fuel & charging stations', typePhrase: 'fuel and charging stations' },
    'charging_station': { bucketKey: 'fuel', bucketLabel: 'Fuel & charging stations', typePhrase: 'fuel and charging stations' },
    
    // Water services
    'water': { bucketKey: 'water', bucketLabel: 'Water services', typePhrase: 'water services' },
    'water_utility': { bucketKey: 'water', bucketLabel: 'Water services', typePhrase: 'water services' },
    'water_point': { bucketKey: 'water', bucketLabel: 'Water services', typePhrase: 'water services' },
    'water_tap': { bucketKey: 'water', bucketLabel: 'Water services', typePhrase: 'water services' },
    'drinking_water': { bucketKey: 'water', bucketLabel: 'Water services', typePhrase: 'water services' },
    
    // Energy & power
    'gas': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    'gas_utility': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    'electricity': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    'energy': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    'energy_supplier': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    'sub_station': { bucketKey: 'energy', bucketLabel: 'Energy & power', typePhrase: 'energy and power services' },
    
    // Waste management
    'waste_basket': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
    'waste_disposal': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
    'waste_transfer_station': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
    'recycling': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
    'compost': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
    
    // Public facilities
    'toilets': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'shower': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'public_bath': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'hand_sanitizing': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'handwashing': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'foot_shower': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    
    // Storage & lockers
    'locker': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    'lockers': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    'left_luggage': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    'cloakroom': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    'dressing_room': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    'parcel_locker': { bucketKey: 'storage', bucketLabel: 'Storage & lockers', typePhrase: 'storage and locker facilities' },
    
    // Other communications and technology
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other communications and technology', typePhrase: 'communications and technology' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'sub_station': 'Electrical substation',
    'waste_basket': 'Waste bin',
    'waste_disposal': 'Waste disposal',
    'waste_transfer_station': 'Waste transfer station',
    'recycling': 'Recycling center',
    'compost': 'Compost facility',
    'toilets': 'Public restroom',
    'shower': 'Public shower',
    'public_bath': 'Public bath',
    'hand_sanitizing': 'Hand sanitizing station',
    'handwashing': 'Handwashing facility',
    'foot_shower': 'Foot shower',
    'locker': 'Locker',
    'lockers': 'Lockers',
    ...
    'turn_stile': 'Turnstile',
    'weighbridge': 'Weighbridge',
    'trolley_bay': 'Shopping cart bay',
    'street_lamp': 'Street lamp',
  };
  
  return fallbacks[cat] || 'Utility service';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Communications & Technology
...
/**
 * Update names for bucket context (no special handling needed for Communications & Technology)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Communications & Technology
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'communications and technology';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'communications and technology';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 communications and technology service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} communications and technology services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Communications & Technology
 ...
 */
function validateOutput(output: UtilitiesInfrastructureOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: UtilitiesInfrastructureOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if communicationsTechnology already exists (unless force)
  if (!options.force && buffet.communicationsTechnology && buffet.communicationsTechnology.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: UtilitiesInfrastructureOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ communicationsTechnology: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: UtilitiesInfrastructureOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (communicationsTechnology already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Communications & Technology Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.communicationsTechnology`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has communicationsTechnology (unless force)
        if (!force && buffet.communicationsTechnology && buffet.communicationsTechnology.trim().length > 0) {

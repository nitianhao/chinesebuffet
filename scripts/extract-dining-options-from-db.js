// Script to extract "Dining options" from additionalInfo field in database
// and populate the diningOptions field
// Run with: node scripts/extract-dining-options-from-db.js [--all]
...
/**
 * Extract "Dining options" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of dining options, or null if not found
 */
function extractDiningOptions(additionalInfo) {
  if (!additionalInfo) {
    return null;
  }
  ...
  // Check if "Dining options" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const diningOptionsKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'dining options' || key === 'Dining options' || key === 'diningOptions'
  );
  ...
  const diningOptionsArray = infoObj[diningOptionsKey];

  if (!Array.isArray(diningOptionsArray) || diningOptionsArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "Brunch": true }, { "Lunch": true }, { "Dinner": true }]
  const diningItems = [];
  
  for (const item of diningOptionsArray) {
    if (typeof item === 'object' && item !== null) {
      ...
      for (const key of keys) {
        if (item[key] === true || item[key]) {
          diningItems.push(key);
        }
      }
    } else if (typeof item === 'string') {
      // If it's already a string, just add it
      diningItems.push(item);
    }
  }
  ...
  return diningItems.join(', ');
}

async function extractDiningOptionsFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Dining options" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Dining options"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithDiningOptions = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const diningOptions = extractDiningOptions(additionalInfo);

        if (!diningOptions || diningOptions.trim() === '') {
          skipped++;
          continue;
        }

        foundWithDiningOptions++;

        // Check if it's already set and different
        const currentDiningOptions = buffet.diningOptions || null;
        if (currentDiningOptions === diningOptions) {
        ...
        updates.push({
          ...
          diningOptions: diningOptions.trim(),
          oldValue: currentDiningOptions
        });
    ...
    console.log(`\n✓ Found ${foundWithDiningOptions} records with "Dining options" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct diningOptions value or have no "Dining options" data.\n');
      return;
    }
      ...
      try {
        const transactions = batch.map(update => 
          db.tx.buffets[update.buffetId].update({
            diningOptions: update.diningOptions
          })
        );
        ...
        for (const update of batch) {
          ...
          console.log(`  → diningOptions: ${update.oldValue || 'null'} → "${update.diningOptions}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Dining options": ${foundWithDiningOptions}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-dining-options-from-db.js --all\n');
    }
...
extractDiningOptionsFromDB().catch(error => {

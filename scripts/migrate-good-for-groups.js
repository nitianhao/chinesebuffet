// Script to migrate goodForGroups from buffets table to structuredData table
// Run with: node scripts/migrate-good-for-groups.js [--all]
...
async function migrateGoodForGroups() {
  try {
    console.log('================================================================================');
    console.log('Migrating goodForGroups to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have goodForGroups set
    const buffets = allBuffets.buffets.filter(b => b.goodForGroups !== null && b.goodForGroups !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with goodForGroups`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with goodForGroups field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.goodForGroups),
              type: 'goodForGroups',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: goodForGroups)`);
        console.log(`  → Value: ${buffet.goodForGroups}\n`);

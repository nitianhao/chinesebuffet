// Script to check imageUrls, inputPlaceId, and kgmid fields
...
const fieldsToCheck = ['imageUrls', 'inputPlaceId', 'kgmid'];
      ...
      if (stats.examples.length > 0) {
        ...
        stats.examples.forEach((example, index) => {
          ...
          if (example.parseError) {
            ...
            console.log(`    Raw value: ${JSON.stringify(example.rawValue).substring(0, 150)}${JSON.stringify(example.rawValue).length > 150 ? '...' : ''}`);
          } else if (example.parsedValue !== null) {
            const valueStr = JSON.stringify(example.parsedValue);
            console.log(`    Parsed value: ${valueStr.substring(0, 200)}${valueStr.length > 200 ? '...' : ''}`);
          } else {
            console.log(`    Raw value: ${example.rawValue.substring(0, 150)}${example.rawValue.length > 150 ? '...' : ''}`);
          }
        });
      } else {

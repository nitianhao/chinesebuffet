/**
 * Generate SEO-optimized "Food & Dining" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.foodDining (JSON string)
 * 
 * Note: This script has very relaxed constraints to allow maximum POIs per buffet.
 * Comprehensive bucketization strategy with detailed categories for maximum informativeness.
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-food-dining.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-food-dining.ts --concurrency 5
 *   npx tsx scripts/generate-poi-food-dining.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-food-dining.ts --resume
 *   npx tsx scripts/generate-poi-food-dining.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Food & Dining';
// Very relaxed constraints for maximum informativeness
const MAX_ITEMS_PER_BUCKET = 12;
const MAX_TOTAL_ITEMS = 30;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'food-dining.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Restaurants
    'restaurant': { bucketKey: 'restaurants', bucketLabel: 'Restaurants', typePhrase: 'restaurants' },
    'fast_food': { bucketKey: 'restaurants', bucketLabel: 'Restaurants', typePhrase: 'restaurants' },
    'food_court': { bucketKey: 'restaurants', bucketLabel: 'Restaurants', typePhrase: 'restaurants' },
    'canteen': { bucketKey: 'restaurants', bucketLabel: 'Restaurants', typePhrase: 'restaurants' },
    'ghost_kitchen': { bucketKey: 'restaurants', bucketLabel: 'Restaurants', typePhrase: 'restaurants' },
    
    // Cafes & Coffee
    'cafe': { bucketKey: 'cafes', bucketLabel: 'Cafes & Coffee', typePhrase: 'cafes and coffee shops' },
    'coffee': { bucketKey: 'cafes', bucketLabel: 'Cafes & Coffee', typePhrase: 'cafes and coffee shops' },
    'tea': { bucketKey: 'cafes', bucketLabel: 'Cafes & Coffee', typePhrase: 'cafes and coffee shops' },
    'juice_bar': { bucketKey: 'cafes', bucketLabel: 'Cafes & Coffee', typePhrase: 'cafes and coffee shops' },
    'juice': { bucketKey: 'cafes', bucketLabel: 'Cafes & Coffee', typePhrase: 'cafes and coffee shops' },
    
    // Bars & Pubs
    'bar': { bucketKey: 'bars', bucketLabel: 'Bars & Pubs', typePhrase: 'bars and pubs' },
    'pub': { bucketKey: 'bars', bucketLabel: 'Bars & Pubs', typePhrase: 'bars and pubs' },
    'biergarten': { bucketKey: 'bars', bucketLabel: 'Bars & Pubs', typePhrase: 'bars and pubs' },
    'alcohol': { bucketKey: 'bars', bucketLabel: 'Bars & Pubs', typePhrase: 'bars and pubs' },
    'wine': { bucketKey: 'bars', bucketLabel: 'Bars & Pubs', typePhrase: 'bars and pubs' },
    
    // Bakeries & Desserts
    'bakery': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'donut': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'pastry': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'dessert': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'ice_cream': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'confectionery': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    'chocolate': { bucketKey: 'bakeries', bucketLabel: 'Bakeries & Desserts', typePhrase: 'bakeries and dessert shops' },
    
    // Specialty Food
    'deli': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'butcher': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'seafood': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'sushi': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'pasta': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'specialty_food': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'health_food': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'cheese': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    'caviar': { bucketKey: 'specialty', bucketLabel: 'Specialty Food', typePhrase: 'specialty food shops' },
    
    // Grocery & Markets
    'grocery': { bucketKey: 'grocery', bucketLabel: 'Grocery & Markets', typePhrase: 'grocery stores and markets' },
    'supermarket': { bucketKey: 'grocery', bucketLabel: 'Grocery & Markets', typePhrase: 'grocery stores and markets' },
    'convenience': { bucketKey: 'grocery', bucketLabel: 'Grocery & Markets', typePhrase: 'grocery stores and markets' },
    'convenience_store': { bucketKey: 'grocery', bucketLabel: 'Grocery & Markets', typePhrase: 'grocery stores and markets' },
    'greengrocer': { bucketKey: 'grocery', bucketLabel: 'Grocery & Markets', typePhrase: 'grocery stores and markets' },
    
    // Food Products
    'dairy': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'nuts': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'edible_seeds': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'spices': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'honey': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'olive_oil': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'frozen_food': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    'dry_food': { bucketKey: 'products', bucketLabel: 'Food Products', typePhrase: 'food product stores' },
    
    // Catering & Services
    'catering': { bucketKey: 'catering', bucketLabel: 'Catering & Services', typePhrase: 'catering and food services' },
    'food_sharing': { bucketKey: 'catering', bucketLabel: 'Catering & Services', typePhrase: 'catering and food services' },
    
    // General Food & Beverages
    'food': { bucketKey: 'general', bucketLabel: 'Food & Beverages', typePhrase: 'food and beverage establishments' },
    'snack': { bucketKey: 'general', bucketLabel: 'Food & Beverages', typePhrase: 'food and beverage establishments' },
    'beverages': { bucketKey: 'general', bucketLabel: 'Food & Beverages', typePhrase: 'food and beverage establishments' },
  };
  
  return bucketMap[cat] || { bucketKey: 'general', bucketLabel: 'Food & Beverages', typePhrase: 'food and beverage establishments' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'specialty_food': 'Specialty food shop',
    ...
    'dairy': 'Dairy shop',
    'nuts': 'Nuts shop',
    'edible_seeds': 'Edible seeds shop',
    'spices': 'Spice shop',
    'honey': 'Honey shop',
    'olive_oil': 'Olive oil shop',
    'frozen_food': 'Frozen food store',
    'dry_food': 'Dry food store',
    'catering': 'Catering service',
    'food_sharing': 'Food sharing service',
    'food': 'Food establishment',
    'snack': 'Snack shop',
    'beverages': 'Beverage shop',
  };
  
  return fallbacks[cat] || 'Food establishment';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Food & Dining
...
/**
 * Update names for bucket context (no special handling needed for Food & Dining)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Food & Dining
  return items;
}
 ...
 */
function validateOutput(output: FoodDiningOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: FoodDiningOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if foodDining already exists (unless force)
  if (!options.force && buffet.foodDining && buffet.foodDining.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ foodDining: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (foodDining already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Food & Dining Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.foodDining`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has foodDining (unless force)
        if (!force && buffet.foodDining && buffet.foodDining.trim().length > 0) {

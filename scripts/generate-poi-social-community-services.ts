/**
 * Generate SEO-optimized "Social & Community Services" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.socialCommunityServices (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-social-community-services.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-social-community-services.ts --concurrency 5
 *   npx tsx scripts/generate-poi-social-community-services.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-social-community-services.ts --resume
 *   npx tsx scripts/generate-poi-social-community-services.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Social & Community Services';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'social-community-services.checkpoint.json');
...
type BuffetRecord = {
  id: string;
  name?: string | null;
  socialCommunityServices?: string | null;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    'public_bookcase': { bucketKey: 'libraries', bucketLabel: 'Little free libraries & book exchanges', typePhrase: 'book exchanges' },
    'give_box': { bucketKey: 'libraries', bucketLabel: 'Little free libraries & book exchanges', typePhrase: 'book exchanges' },
    
    // Food sharing & community pantries
    'food_sharing': { bucketKey: 'food_support', bucketLabel: 'Food sharing & community pantries', typePhrase: 'community pantries' },
    'donation_dropoff_location': { bucketKey: 'food_support', bucketLabel: 'Food sharing & community pantries', typePhrase: 'community pantries' },
    
    // Childcare & learning centers
    'childcare': { bucketKey: 'childcare', bucketLabel: 'Childcare & learning centers', typePhrase: 'childcare centers' },
    'foster_care_services': { bucketKey: 'childcare', bucketLabel: 'Childcare & learning centers', typePhrase: 'childcare centers' },
    
    // Blood donation centers
    'blood_donation': { bucketKey: 'blood', bucketLabel: 'Blood donation centers', typePhrase: 'blood donation centers' },
    
    // Social facilities & assisted living
    'social_facility': { bucketKey: 'facilities', bucketLabel: 'Social facilities & assisted living', typePhrase: 'support facilities' },
    'senior_care': { bucketKey: 'facilities', bucketLabel: 'Social facilities & assisted living', typePhrase: 'support facilities' },
    'social_service': { bucketKey: 'facilities', bucketLabel: 'Social facilities & assisted living', typePhrase: 'support facilities' },
    'social_services': { bucketKey: 'facilities', bucketLabel: 'Social facilities & assisted living', typePhrase: 'support facilities' },
    
    // Community & employment centers
    'social_centre': { bucketKey: 'community_centers', bucketLabel: 'Community & employment centers', typePhrase: 'community centers' },
    'community_centre': { bucketKey: 'community_centers', bucketLabel: 'Community & employment centers', typePhrase: 'community centers' },
    'social_club': { bucketKey: 'community_centers', bucketLabel: 'Community & employment centers', typePhrase: 'community centers' },
    
    // Community associations
    'association': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'ngo': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'charity': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'foundation': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'nonprofit': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'non_profit': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'fraternity': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'sorority': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'cooperative': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    
    // Labor organizations (map to associations)
    'union': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'labor_union': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
    'trade_union': { bucketKey: 'associations', bucketLabel: 'Community associations', typePhrase: 'community associations' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other community services', typePhrase: 'community services' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  // Special handling for social_facility based on tags
  if (cat === 'social_facility') {
    const facilityFor = (tagsObj['social_facility:for'] || tagsObj['social_facility'] || '').toLowerCase();
    if (facilityFor === 'senior' || tagsObj.group_home) {
      return 'Senior living facility';
    }
    if (facilityFor === 'veterans' || tagsObj.veterans) {
      return 'Veterans support center';
    }
    return 'Community support facility';
  }
  
  const fallbacks: Record<string, string> = {
    'public_bookcase': 'Book exchange',
    'food_sharing': 'Community pantry',
    ...
    'ngo': 'Community association',
    'charity': 'Community association',
    'foundation': 'Community association',
    'nonprofit': 'Community association',
    'non_profit': 'Community association',
    'senior_care': 'Senior living facility',
    'foster_care_services': 'Childcare center',
    'social_club': 'Community center',
    'social_service': 'Community support facility',
    'social_services': 'Community support facility',
    'cooperative': 'Community association',
    'union': 'Community association',
    'labor_union': 'Community association',
    'trade_union': 'Community association',
    'fraternity': 'Community association',
    'sorority': 'Community association',
    'give_box': 'Book exchange',
    'donation_dropoff_location': 'Community pantry',
  };
  
  return fallbacks[cat] || 'Community service';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // Special dedupe: For public_bookcase with "little free library" name and no address,
  // keep only the closest one
  const publicBookcases = sorted.filter(poi => {
    const category = (poi.category || '').toLowerCase().trim();
    if (category !== 'public_bookcase') return false;
    ...
    return normalizedName === 'little free library' && !addressText;
  });
  
  if (publicBookcases.length > 1) {
    // Keep only the closest one, remove others
    const closest = publicBookcases[0];
    const toRemove = new Set(publicBookcases.slice(1).map(p => p.id));
    sorted = sorted.filter(poi => !toRemove.has(poi.id));
  }
...
/**
 * Update names for public_bookcase items based on bucket context
 * If bucket already has "Little Free Library", use "Little Free Library (book exchange)" for generic ones
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // Check if this bucket has any "Little Free Library" items
  const hasLFL = items.some(item => 
    item.category === 'public_bookcase' && 
    normalizeName(item.name) === 'little free library'
  );
  
  // Update generic book exchange names if LFL exists in bucket
  if (hasLFL) {
    return items.map(item => {
      if (item.category === 'public_bookcase' && normalizeName(item.name) === 'book exchange') {
        return { ...item, name: 'Little Free Library (book exchange)' };
      }
      return item;
    });
  }
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'community services';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'community services';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  const serviceWord = poiCount === 1 ? 'service' : 'services';
  // Single-item: remove "about" hedge
  if (poiCount === 1) {
    const sentence1 = `1 social & community service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} social & community services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // Option 3: Intent hook (only when poiCount <= 3 and relevant bucket is dominant/closest)
  if (!sentence2 && poiCount <= 3) {
    // Food sharing hook: only if food_support is dominant or closest
    if (bucketCounts.food_support > 0 && (closestBucketKey === 'food_support' || (bucketCounts.food_support >= (bucketCounts.libraries || 0)))) {
      sentence2 = 'Useful if you\'re looking for a nearby community pantry.';
    }
    // Childcare hook: only if childcare is dominant or closest
    else if (bucketCounts.childcare > 0 && (closestBucketKey === 'childcare' || (bucketCounts.childcare >= (bucketCounts.libraries || 0)))) {
      sentence2 = 'Helpful for families looking for childcare options.';
    }
    // Blood donation hook: only if blood is dominant or closest
    else if (bucketCounts.blood > 0 && (closestBucketKey === 'blood' || (bucketCounts.blood >= (bucketCounts.libraries || 0)))) {
      sentence2 = 'Useful if you\'re looking for a blood donation center.';
    }
  }
 ...
 */
function validateOutput(output: SocialCommunityOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: SocialCommunityOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if socialCommunityServices already exists (unless force)
  if (!options.force && buffet.socialCommunityServices && buffet.socialCommunityServices.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: SocialCommunityOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ socialCommunityServices: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (socialCommunityServices already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Social & Community Services Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.socialCommunityServices`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has socialCommunityServices (unless force)
        if (!force && buffet.socialCommunityServices && buffet.socialCommunityServices.trim().length > 0) {

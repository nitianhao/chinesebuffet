/**
 * Generate SEO-optimized "Home Improvement & Garden" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.homeImprovementGarden (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-home-improvement-garden.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-home-improvement-garden.ts --concurrency 5
 *   npx tsx scripts/generate-poi-home-improvement-garden.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-home-improvement-garden.ts --resume
 *   npx tsx scripts/generate-poi-home-improvement-garden.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Home Improvement & Garden';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'home-improvement-garden.checkpoint.json');
...
type BuffetRecord = {
  ...
  name?: string | null;
  placeId?: string | null;
  homeImprovementGarden?: string | null;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    // Other home improvement and garden
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other home improvement and garden', typePhrase: 'home improvement and garden' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Home Improvement & Garden
...
/**
 * Update names for bucket context (no special handling needed for Home Improvement & Garden)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Home Improvement & Garden
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'home improvement and garden';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'home improvement and garden';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 home improvement and garden service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} home improvement and garden services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Home Improvement & Garden
...
async function processBuffet(
  ...
  // Skip if homeImprovementGarden already exists (unless force)
  if (!options.force && buffet.homeImprovementGarden && buffet.homeImprovementGarden.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ homeImprovementGarden: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (homeImprovementGarden already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Home Improvement & Garden Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.homeImprovementGarden`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has homeImprovementGarden (unless force)
        if (!force && buffet.homeImprovementGarden && buffet.homeImprovementGarden.trim().length > 0) {

// Script to migrate planning from buffets table to structuredData table
// Run with: node scripts/migrate-planning-to-structured-data.js [--all]
...
async function migratePlanning() {
  ...
  console.log('Migrating planning to structuredData table');
  console.log('='.repeat(80));
  console.log(`Mode: ${migrateAll ? 'ALL records' : 'TEST (1 record only)'}\n`);

  try {
    // Query buffets - we'll filter for those with planning in JavaScript
    console.log('Querying buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    // Fetch buffets in batches and filter for those with planning
    while (true) {
      const query = {
        buffets: {
          $: {
      ...
      // Filter for buffets that have planning
    ...
    if (buffets.length === 0) {
      console.log('✓ No buffets found with planning');
      return;
    }

    console.log(`✓ Found ${buffets.length} buffet(s) with planning\n`);
    ...
    for (let i = 0; i < buffets.length; i++) {
      ...
      try {
        // Check if structuredData already exists for this buffet with type 'planning'
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {}
        ...
        const hasPlanning = existingStructuredData.some(sd => sd.type === 'planning');
        ...
        // Parse the planning field
        // Note: planning is described as "Planning information" in the schema
        let planningData = null;
        if (buffet.planning) {
          try {
            if (typeof buffet.planning === 'string') {
              // First, try to parse as JSON
              try {
                planningData = JSON.parse(buffet.planning);
              } catch (jsonError) {
                // If not valid JSON, check if it's comma-separated
                if (buffet.planning.includes(',')) {
                  // Split by comma, trim each item, and create an array
                  planningData = buffet.planning
                    .split(',')
                    .map(item => item.trim())
                    .filter(item => item.length > 0);
                } else {
                  // Single value, create array with one item
                  planningData = [buffet.planning.trim()];
                }
              }
            } else if (Array.isArray(buffet.planning)) {
              // Already an array
              planningData = buffet.planning;
            } else {
              // Already an object
            ...
            console.error(`⚠ Error parsing planning for buffet "${buffet.name}":`, parseError.message);
            errors++;
            continue;
          }
        }
        ...
        if (Array.isArray(planningData)) {
          dataString = JSON.stringify(planningData);
        ...
        if (migrateAll) {
          ...
          console.log(`  → Created structuredData record (id: ${structuredDataId}, type: planning)`);
        ...
        if (!migrateAll && migrated === 1) {
          ...
          console.log('  node scripts/migrate-planning-to-structured-data.js --all\n');
          break;
        }

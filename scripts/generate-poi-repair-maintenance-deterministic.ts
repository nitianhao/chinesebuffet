/**
 * Generate SEO-optimized "Repair & Maintenance Services" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.repairMaintenance (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-repair-maintenance-deterministic.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-repair-maintenance-deterministic.ts --concurrency 5
 *   npx tsx scripts/generate-poi-repair-maintenance-deterministic.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-repair-maintenance-deterministic.ts --resume
 *   npx tsx scripts/generate-poi-repair-maintenance-deterministic.ts --force --limit 100
 */
...
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'repair-maintenance-deterministic.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other repair and maintenance services', typePhrase: 'repair and maintenance services' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Repair & Maintenance Services
...
/**
 * Update names for bucket context (no special handling needed for Repair & Maintenance Services)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Repair & Maintenance Services
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'repair and maintenance services';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'repair and maintenance services';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 repair and maintenance services service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} repair and maintenance services services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Repair & Maintenance Services
...
async function processBuffet(
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ repairMaintenance: jsonString })]);
  }
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Repair & Maintenance Services Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.repairMaintenance`);

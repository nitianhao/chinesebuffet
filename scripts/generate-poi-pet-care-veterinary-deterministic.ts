/**
 * Generate SEO-optimized "Pet Care & Veterinary" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.petCareVeterinary (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-pet-care-veterinary-deterministic.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-pet-care-veterinary-deterministic.ts --concurrency 5
 *   npx tsx scripts/generate-poi-pet-care-veterinary-deterministic.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-pet-care-veterinary-deterministic.ts --resume
 *   npx tsx scripts/generate-poi-pet-care-veterinary-deterministic.ts --force --limit 100
 */
...
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'pet-care-veterinary.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    // Other pet care and veterinary
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other pet care and veterinary', typePhrase: 'pet care and veterinary' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Pet Care & Veterinary
...
/**
 * Update names for bucket context (no special handling needed for Pet Care & Veterinary)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Pet Care & Veterinary
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'pet care and veterinary';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'pet care and veterinary';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 pet care and veterinary service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} pet care and veterinary services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Pet Care & Veterinary
...
async function processBuffet(
  ...
  // Skip if petCareVeterinary already exists (unless force)
  if (!options.force && buffet.petCareVeterinary && buffet.petCareVeterinary.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ petCareVeterinary: jsonString })]);
  }
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Pet Care & Veterinary Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.petCareVeterinary`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has petCareVeterinary (unless force)
        if (!force && buffet.petCareVeterinary && buffet.petCareVeterinary.trim().length > 0) {

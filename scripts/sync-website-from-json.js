// Script to sync website data from apify-big-cities.json to instantDB buffet table
// Run with: node scripts/sync-website-from-json.js [--all]
...
async function syncWebsites() {
  ...
  console.log('Syncing website data from JSON to instantDB buffet table');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Filter records that have website data
    const recordsWithWebsite = jsonData.filter(record => {
    ...
    console.log(`✓ Found ${recordsWithWebsite.length} records with website data\n`);

    if (recordsWithWebsite.length === 0) {
      console.log('No records with website data found. Exiting.');
      return;
    }
    ...
    const recordsToProcess = migrateAll 
      ? recordsWithWebsite 
      : recordsWithWebsite.slice(0, testLimit);
    ...
    for (const record of recordsToProcess) {
      if (record.PlaceID && record.website) {
        websiteMap.set(record.PlaceID, record.website.trim());
      }
    }

    console.log(`✓ Created lookup map with ${websiteMap.size} PlaceIDs\n`);

    // Fetch buffets that match these PlaceIDs
    console.log('Fetching matching buffets from database...');
    const placeIds = Array.from(websiteMap.keys());
    ...
    // Prepare updates (override existing website values as requested)
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    const updates = [];
    ...
    for (const buffet of allBuffets) {
      try {
        ...
        const newWebsite = websiteMap.get(placeId);

        if (!newWebsite) {
          console.log(`⏭ Skipping buffet "${buffet.name}" (placeId: ${placeId}) - no website in JSON`);
          skipped++;
          continue;
        }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → website: ${update.oldValue || 'null'} → ${update.website}`);
          updated++;
        }
    ...
    if (!migrateAll && updated > 0) {
      ...
      console.log('  node scripts/sync-website-from-json.js --all\n');
    }

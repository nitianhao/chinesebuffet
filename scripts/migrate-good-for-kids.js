// Script to migrate goodForKids from buffets table to structuredData table
// Run with: node scripts/migrate-good-for-kids.js [--all]
...
async function migrateGoodForKids() {
  try {
    console.log('================================================================================');
    console.log('Migrating goodForKids to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have goodForKids set
    const buffets = allBuffets.buffets.filter(b => b.goodForKids !== null && b.goodForKids !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with goodForKids`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with goodForKids field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.goodForKids),
              type: 'goodForKids',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: goodForKids)`);
        console.log(`  → Value: ${buffet.goodForKids}\n`);

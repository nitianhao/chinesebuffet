/**
 * Generate SEO-optimized "Agricultural & Farming" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.agriculturalFarming (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-agricultural-farming.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-agricultural-farming.ts --concurrency 5
 *   npx tsx scripts/generate-poi-agricultural-farming.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-agricultural-farming.ts --resume
 *   npx tsx scripts/generate-poi-agricultural-farming.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Agricultural & Farming';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'agricultural-farming.checkpoint.json');
...
type BuffetRecord = {
  ...
  name?: string | null;
  placeId?: string | null;
  agriculturalFarming?: string | null;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    // Other agricultural and farming
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other agricultural and farming', typePhrase: 'agricultural and farming' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Agricultural & Farming
...
/**
 * Update names for bucket context (no special handling needed for Agricultural & Farming)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Agricultural & Farming
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'agricultural and farming';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'agricultural and farming';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 agricultural and farming service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} agricultural and farming services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Agricultural & Farming
...
async function processBuffet(
  ...
  // Skip if agriculturalFarming already exists (unless force)
  if (!options.force && buffet.agriculturalFarming && buffet.agriculturalFarming.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ agriculturalFarming: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (agriculturalFarming already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Agricultural & Farming Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.agriculturalFarming`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has agriculturalFarming (unless force)
        if (!force && buffet.agriculturalFarming && buffet.agriculturalFarming.trim().length > 0) {

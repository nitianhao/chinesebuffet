/**
 * Generate SEO-optimized "Education & Learning" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.educationLearning (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-education-learning.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-education-learning.ts --concurrency 5
 *   npx tsx scripts/generate-poi-education-learning.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-education-learning.ts --resume
 *   npx tsx scripts/generate-poi-education-learning.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Education & Learning';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'education-learning.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    'school': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'university': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'college': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'kindergarten': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'prep_school': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'educational_institution': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    'education': { bucketKey: 'schools', bucketLabel: 'Schools & universities', typePhrase: 'schools and universities' },
    
    // Specialized schools
    'language_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'driving_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'music_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'dancing_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'art_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'acting_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'flight_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'ski_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'swimming_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'first_aid_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    'trade_school': { bucketKey: 'specialized', bucketLabel: 'Specialized schools', typePhrase: 'specialized schools' },
    
    // Tutoring & training
    'tutoring': { bucketKey: 'tutoring', bucketLabel: 'Tutoring & training', typePhrase: 'tutoring and training services' },
    'tutor': { bucketKey: 'tutoring', bucketLabel: 'Tutoring & training', typePhrase: 'tutoring and training services' },
    'training': { bucketKey: 'tutoring', bucketLabel: 'Tutoring & training', typePhrase: 'tutoring and training services' },
    'lessons': { bucketKey: 'tutoring', bucketLabel: 'Tutoring & training', typePhrase: 'tutoring and training services' },
    
    // Libraries & archives
    'library': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries and archives' },
    'archive': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries and archives' },
    'archives': { bucketKey: 'libraries', bucketLabel: 'Libraries & archives', typePhrase: 'libraries and archives' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other education & learning venues', typePhrase: 'education & learning venues' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  return fallbacks[cat] || 'Education & learning venue';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Education & Learning
...
/**
 * Update names for bucket context (no special handling needed for Education & Learning)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Education & Learning
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'education & learning venues';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'education & learning venues';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  const venueWord = poiCount === 1 ? 'venue' : 'venues';
  // Single-item: remove "about" hedge
  if (poiCount === 1) {
    const sentence1 = `1 education & learning venue is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} education & learning venues are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Education & Learning
 ...
 */
function validateOutput(output: EducationLearningOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: EducationLearningOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if educationLearning already exists (unless force)
  if (!options.force && buffet.educationLearning && buffet.educationLearning.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: EducationLearningOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ educationLearning: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: EducationLearningOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (educationLearning already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Education & Learning Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.educationLearning`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has educationLearning (unless force)
        if (!force && buffet.educationLearning && buffet.educationLearning.trim().length > 0) {

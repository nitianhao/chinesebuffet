// Script to migrate menu_url from yelp-restaurant-mapping.json to buffets.menuUrl
// Run with: node scripts/migrate-menu-url.js [--all]
...
async function migrateMenuUrl() {
  console.log('='.repeat(80));
  console.log('Migrating menu_url from Yelp mapping to buffets.menuUrl');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Filter entries that have menu_url (including empty strings)
    const entriesWithMenuUrl = entries.filter(([key, entry]) => {
      return entry?.yelp?.details?.attributes?.menu_url !== undefined && 
             entry?.yelp?.details?.attributes?.menu_url !== null &&
             entry?.yelp?.details?.attributes?.menu_url !== '';
    });

    console.log(`✓ Found ${entriesWithMenuUrl.length} entries with menu_url data\n`);

    if (entriesWithMenuUrl.length === 0) {
      console.log('No entries with menu_url found. Exiting.');
      return;
    }
    ...
    const entriesToProcess = migrateAll 
      ? entriesWithMenuUrl 
      : entriesWithMenuUrl.slice(0, testLimit);

    console.log(`Processing ${entriesToProcess.length} entries...\n`);

    // Create a map of placeId -> menu_url for quick lookup
    ...
    for (const [key, entry] of entriesToProcess) {
      ...
      // Include all non-empty string values
      if (menuUrl && typeof menuUrl === 'string' && menuUrl.trim() !== '') {
        ...
        if (placeId) {
          menuUrlMap.set(placeId, menuUrl.trim());
        }
      }
    }

    console.log(`✓ Created lookup map with ${menuUrlMap.size} placeIds\n`);

    // Fetch buffets that match these placeIds
    console.log('Fetching matching buffets from database...');
    const placeIds = Array.from(menuUrlMap.keys());
    ...
    for (const buffet of allBuffets) {
      try {
        ...
        const menuUrl = menuUrlMap.get(placeId);

        if (!menuUrl) {
          console.log(`⏭ Skipping buffet "${buffet.name}" (placeId: ${placeId}) - no menu_url in mapping`);
          skipped++;
          continue;
        }

        // Check if already has the same value
        if (buffet.menuUrl === menuUrl) {
          console.log(`⏭ Skipping buffet "${buffet.name}" (placeId: ${placeId}) - already has menuUrl: ${menuUrl}`);
          skipped++;
          continue;
        }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → menuUrl: ${update.oldValue || 'null'} → ${update.menuUrl}`);
          updated++;
        }
    ...
    if (!migrateAll && updated > 0) {
      ...
      console.log('  node scripts/migrate-menu-url.js --all\n');
    }

// Script to migrate outdoorSeating from buffets table to structuredData table
// Run with: node scripts/migrate-outdoor-seating.js [--all]
...
async function migrateOutdoorSeating() {
  try {
    console.log('================================================================================');
    console.log('Migrating outdoorSeating to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have outdoorSeating set
    const buffets = allBuffets.buffets.filter(b => b.outdoorSeating !== null && b.outdoorSeating !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with outdoorSeating`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with outdoorSeating field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.outdoorSeating),
              type: 'outdoorSeating',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: outdoorSeating)`);
        console.log(`  → Value: ${buffet.outdoorSeating}\n`);

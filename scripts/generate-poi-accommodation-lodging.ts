/**
 * Generate SEO-optimized "Accommodation & Lodging" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.accommodationLodging (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-accommodation-lodging.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-accommodation-lodging.ts --concurrency 5
 *   npx tsx scripts/generate-poi-accommodation-lodging.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-accommodation-lodging.ts --resume
 *   npx tsx scripts/generate-poi-accommodation-lodging.ts --force --limit 100
 */
...
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'accommodation-lodging.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    // Other accommodation and lodging
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other accommodation and lodging', typePhrase: 'accommodation and lodging' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Accommodation & Lodging
...
/**
 * Update names for bucket context (no special handling needed for Accommodation & Lodging)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Accommodation & Lodging
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'accommodation and lodging';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'accommodation and lodging';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 accommodation and lodging service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} accommodation and lodging services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Accommodation & Lodging
...
async function processBuffet(
  ...
  // Skip if accommodationLodging already exists (unless force)
  if (!options.force && buffet.accommodationLodging && buffet.accommodationLodging.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ accommodationLodging: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (accommodationLodging already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Accommodation & Lodging Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.accommodationLodging`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has accommodationLodging (unless force)
        if (!force && buffet.accommodationLodging && buffet.accommodationLodging.trim().length > 0) {

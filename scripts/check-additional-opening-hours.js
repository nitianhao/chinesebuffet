// Script to check additionalOpeningHours field
...
async function checkAdditionalOpeningHours() {
  console.log('Checking additionalOpeningHours field...\n');
  ...
  try {
    ...
    // Analyze additionalOpeningHours
    const stats = {
      appearsInRecords: 0,
      nonNull: 0,
      nonEmpty: 0,
    ...
    allBuffets.forEach(buffet => {
      if ('additionalOpeningHours' in buffet) {
        stats.appearsInRecords++;
        if (buffet.additionalOpeningHours !== null && buffet.additionalOpeningHours !== undefined) {
          stats.nonNull++;
          const value = buffet.additionalOpeningHours;
          let isEmpty = false;
    ...
    console.log('additionalOpeningHours Field Analysis');
    console.log('='.repeat(80));
    ...
    if (stats.examples.length > 0) {
      ...
      stats.examples.forEach((example, index) => {
        ...
        console.log(`    Raw value type: ${typeof example.rawValue}`);
        if (example.parseError) {
          ...
          console.log(`    Parsed value: ${JSON.stringify(example.parsedValue, null, 2).substring(0, 500)}${JSON.stringify(example.parsedValue, null, 2).length > 500 ? '...' : ''}`);
        } else {
          console.log(`    Raw value: ${JSON.stringify(example.rawValue).substring(0, 200)}${JSON.stringify(example.rawValue).length > 200 ? '...' : ''}`);
        }
      });
    } else {
      console.log('\n  No non-empty values found for additionalOpeningHours');
    }

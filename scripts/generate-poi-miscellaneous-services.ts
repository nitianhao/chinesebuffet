/**
 * Generate SEO-optimized "Miscellaneous Services" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.miscellaneousServices (JSON string)
 * 
 * Note: This script has relaxed constraints to allow more POIs per buffet than usual.
 * Miscellaneous Services is a catch-all category for POIs that don't fit other groups.
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-miscellaneous-services.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-miscellaneous-services.ts --concurrency 5
 *   npx tsx scripts/generate-poi-miscellaneous-services.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-miscellaneous-services.ts --resume
 *   npx tsx scripts/generate-poi-miscellaneous-services.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Miscellaneous Services';
...
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'miscellaneous-services.checkpoint.json');
...
type MiscellaneousServicesOutput = {
  summary: string;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  // Since Miscellaneous Services is a catch-all category, we'll use a generic bucketization
  // that groups by category name or uses generic buckets
  // For unknown categories, we'll use the category name as the bucket key (normalized)
  
  // Normalize category name for bucket key (replace underscores with spaces, capitalize)
  const normalizedCat = cat.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  
  // Use a generic "Other services" bucket for all miscellaneous items
  // The category name will be preserved in the item data
  return { 
    bucketKey: 'other', 
    bucketLabel: 'Other services', 
    typePhrase: 'miscellaneous services' 
  };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  // For Miscellaneous Services, create a human-friendly name from the category
  // Replace underscores with spaces and capitalize words
  if (!cat) {
    return 'Service';
  }
  
  const friendlyName = cat
    .replace(/_/g, ' ')
    .replace(/\b\w/g, l => l.toUpperCase());
  
  return friendlyName || 'Service';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Miscellaneous Services
...
/**
 * Update names for bucket context (no special handling needed for Miscellaneous Services)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Miscellaneous Services
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'miscellaneous services';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'miscellaneous services';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} miscellaneous services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Miscellaneous Services
 ...
 */
function validateOutput(output: MiscellaneousServicesOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: MiscellaneousServicesOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if miscellaneousServices already exists (unless force)
  if (!options.force && buffet.miscellaneousServices && buffet.miscellaneousServices.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: MiscellaneousServicesOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ miscellaneousServices: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: MiscellaneousServicesOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (miscellaneousServices already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Miscellaneous Services Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.miscellaneousServices`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has miscellaneousServices (unless force)
        if (!force && buffet.miscellaneousServices && buffet.miscellaneousServices.trim().length > 0) {

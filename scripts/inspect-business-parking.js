...
async function inspectBusinessParking() {
  ...
  try {
    ...
    console.log('Fetching businessParking records...');
    
    // Fetch businessParking records (limit to a sample)
    ...
    console.log(`Found ${records.length} businessParking records (sampling first 100)\n`);
        ...
        try {
          ...
          keys.forEach(key => {
            ...
            if (!valueExamples[key]) {
              valueExamples[key] = parsedData[key];
            }
          });
        } catch (e) {
    ...
    // Output single line for copy-paste
    console.log('\n=== Copy-Paste Format ===');

// Script to sync additionalInfo data from apify-big-cities.json to instantDB buffet table
// Overrides existing values if already filled out
// Run with: node scripts/sync-additional-info-from-json.js [--all]
...
async function syncAdditionalInfo() {
  console.log('='.repeat(80));
  console.log('Syncing additionalInfo data from JSON to instantDB buffet table');
  ...
  try {
    ...
    // Filter records that have additionalInfo data
    const recordsWithAdditionalInfo = jsonData.filter(record => {
      const info = record.additionalInfo;
      return info && 
             info !== 'null' &&
             info !== null &&
             ((typeof info === 'object' && (Array.isArray(info) ? info.length > 0 : Object.keys(info).length > 0)) ||
              (typeof info === 'string' && info.trim() !== '' && info.trim() !== 'null')) &&
             record.PlaceID;
    });

    console.log(`✓ Found ${recordsWithAdditionalInfo.length} records with additionalInfo data\n`);

    if (recordsWithAdditionalInfo.length === 0) {
      console.log('No records with additionalInfo data found. Exiting.');
      return;
    }
    ...
    const recordsToProcess = migrateAll 
      ...
      : recordsWithAdditionalInfo.slice(0, testLimit);

    console.log(`Processing ${recordsToProcess.length} records...\n`);

    // Create a map of PlaceID -> additionalInfo (as JSON string) for quick lookup
    ...
    for (const record of recordsToProcess) {
      if (record.PlaceID && record.additionalInfo) {
        ...
        if (typeof record.additionalInfo === 'string') {
          try {
            // Try to parse to validate it's valid JSON
            JSON.parse(record.additionalInfo);
            infoValue = record.additionalInfo.trim();
          } catch (e) {
            // If parsing fails, skip this record
            console.warn(`⚠ Skipping invalid JSON string for PlaceID ${record.PlaceID}`);
            continue;
          }
        } else if (typeof record.additionalInfo === 'object') {
          // Convert object/array to JSON string
          infoValue = JSON.stringify(record.additionalInfo);
        } else {
          continue;
        }
        
        additionalInfoMap.set(record.PlaceID, infoValue);
      }
    }

    console.log(`✓ Created lookup map with ${additionalInfoMap.size} PlaceIDs\n`);

    // Fetch buffets that match these PlaceIDs
    console.log('Fetching matching buffets from database...');
    const placeIds = Array.from(additionalInfoMap.keys());
    ...
    for (const buffet of allBuffets) {
      try {
        ...
        const newAdditionalInfo = additionalInfoMap.get(placeId);
        ...
        const oldValue = buffet.additionalInfo || null;
        updates.push({
          buffetId: buffet.id,
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          if (update.oldValue) {
            try {
              ...
              if (typeof parsed === 'object' && !Array.isArray(parsed)) {
                ...
                oldValuePreview = `{${keys.join(', ')}, ...} (${Object.keys(parsed).length} keys)`;
              } else {
          ...
          try {
            const parsed = typeof update.additionalInfo === 'string' ? JSON.parse(update.additionalInfo) : update.additionalInfo;
            if (typeof parsed === 'object' && !Array.isArray(parsed)) {
              const keys = Object.keys(parsed).slice(0, 3);
              newValuePreview = `{${keys.join(', ')}, ...} (${Object.keys(parsed).length} keys)`;
            } else {
              newValuePreview = update.additionalInfo.substring(0, 60) + '...';
            }
          } catch (e) {
            newValuePreview = update.additionalInfo.substring(0, 60) + '...';
          }
          ...
          console.log(`  → additionalInfo: ${oldValuePreview} → ${newValuePreview}`);
          updated++;
        }
          ...
          try {
            ...
            try {
              ...
              if (typeof parsed === 'object' && !Array.isArray(parsed)) {
                ...
                newValuePreview = update.additionalInfo.substring(0, 40) + '...';
              }
            } catch (e) {
              newValuePreview = update.additionalInfo.substring(0, 40) + '...';
            }
...
syncAdditionalInfo().catch(error => {

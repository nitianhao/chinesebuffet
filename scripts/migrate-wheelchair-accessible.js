// Script to migrate wheelchairAccessible from buffets table to structuredData table
// Run with: node scripts/migrate-wheelchair-accessible.js [--all]
...
async function migrateWheelchairAccessible() {
  try {
    console.log('================================================================================');
    console.log('Migrating wheelchairAccessible to structuredData table');
    console.log('================================================================================');
    ...
    // Filter buffets that have wheelchairAccessible set
    const buffets = allBuffets.buffets.filter(b => b.wheelchairAccessible !== null && b.wheelchairAccessible !== undefined);

    console.log(`✓ Found ${buffets.length} buffet(s) with wheelchairAccessible`);
    
    if (buffets.length === 0) {
      console.log('No buffets found with wheelchairAccessible field. Exiting.');
      return;
    }
      ...
      try {
        ...
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {
              $: { where: { type: 'wheelchairAccessible' } }
            }
          }
        };
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.wheelchairAccessible),
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: wheelchairAccessible)`);
        console.log(`  → Value: ${buffet.wheelchairAccessible}\n`);

// Script to check placesTags field

const { init } = require('@instantdb/admin');
const fs = require('fs');
const path = require('path');

// Load environment variables
const envPaths = [
  path.join(__dirname, '../.env.local'),
  path.join(process.cwd(), '.env.local'),
  path.join(__dirname, '../env.local.txt'),
  path.join(process.cwd(), 'env.local.txt'),
];

for (const envPath of envPaths) {
  try {
    if (fs.existsSync(envPath)) {
      const envFile = fs.readFileSync(envPath, 'utf8');
      envFile.split('\n').forEach(line => {
        const match = line.match(/^([^=:#]+)=(.*)$/);
        if (match) {
          const key = match[1].trim();
          const value = match[2].trim().replace(/^["']|["']$/g, '');
          if (!process.env[key]) {
            process.env[key] = value;
          }
        }
      });
      break;
    }
  } catch (error) {
    // Continue
  }
}

if (!process.env.INSTANT_ADMIN_TOKEN) {
  console.error('Error: INSTANT_ADMIN_TOKEN environment variable is required');
  process.exit(1);
}

const schema = require('../src/instant.schema.ts');

const db = init({
  appId: process.env.NEXT_PUBLIC_INSTANT_APP_ID || process.env.INSTANT_APP_ID || '709e0e09-3347-419b-8daa-bad6889e480d',
  adminToken: process.env.INSTANT_ADMIN_TOKEN,
  schema: schema.default || schema,
});

async function checkPlacesTags() {
  console.log('Checking placesTags field...\n');

  try {
    console.log('Fetching all buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    while (true) {
      const result = await db.query({
        buffets: {
          $: {
            limit: limit,
            offset: offset,
          }
        }
      });
      
      const buffets = result.buffets || [];
      if (buffets.length === 0) break;
      
      allBuffets = allBuffets.concat(buffets);
      if (allBuffets.length % 2000 === 0) {
        console.log(`  Fetched ${allBuffets.length} buffets...`);
      }
      
      if (buffets.length < limit) break;
      offset += limit;
    }

    console.log(`\nTotal buffets: ${allBuffets.length}\n`);

    // Analyze placesTags
    const stats = {
      appearsInRecords: 0,
      nonNull: 0,
      nonEmpty: 0,
      examples: [],
      uniqueValues: new Set()
    };

    allBuffets.forEach(buffet => {
      if ('placesTags' in buffet) {
        stats.appearsInRecords++;
        if (buffet.placesTags !== null && buffet.placesTags !== undefined) {
          stats.nonNull++;
          const value = buffet.placesTags;
          let isEmpty = false;
          
          if (typeof value === 'string') {
            const trimmed = value.trim();
            // Check if it's not an empty string or the string "null"
            if (trimmed !== '' && trimmed !== 'null' && trimmed !== '[]' && trimmed !== '{}') {
              // For JSON strings, try to parse and check if it's meaningful
              if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
                try {
                  const parsed = JSON.parse(trimmed);
                  if (Array.isArray(parsed)) {
                    isEmpty = parsed.length === 0;
                    if (!isEmpty) {
                      // Track unique tags
                      parsed.forEach(tag => {
                        if (typeof tag === 'string') {
                          stats.uniqueValues.add(tag);
                        }
                      });
                    }
                  } else if (typeof parsed === 'object' && parsed !== null) {
                    isEmpty = Object.keys(parsed).length === 0;
                  }
                  if (!isEmpty) {
                    stats.nonEmpty++;
                    if (stats.examples.length < 5) {
                      stats.examples.push({
                        buffetId: buffet.id,
                        buffetName: buffet.name || 'N/A',
                        rawValue: value,
                        parsedValue: parsed
                      });
                    }
                  }
                } catch (e) {
                  // If it's not valid JSON but has content, count it
                  stats.nonEmpty++;
                  if (stats.examples.length < 5) {
                    stats.examples.push({
                      buffetId: buffet.id,
                      buffetName: buffet.name || 'N/A',
                      rawValue: value,
                      parsedValue: null,
                      parseError: e.message
                    });
                  }
                }
              } else {
                // Non-JSON string with content
                stats.nonEmpty++;
                stats.uniqueValues.add(trimmed);
                if (stats.examples.length < 5) {
                  stats.examples.push({
                    buffetId: buffet.id,
                    buffetName: buffet.name || 'N/A',
                    rawValue: value,
                    parsedValue: null
                  });
                }
              }
            }
          } else if (Array.isArray(value) && value.length > 0) {
            stats.nonEmpty++;
            value.forEach(tag => {
              if (typeof tag === 'string') {
                stats.uniqueValues.add(tag);
              }
            });
            if (stats.examples.length < 5) {
              stats.examples.push({
                buffetId: buffet.id,
                buffetName: buffet.name || 'N/A',
                rawValue: value,
                parsedValue: value
              });
            }
          } else if (typeof value === 'object' && value !== null && Object.keys(value).length > 0) {
            stats.nonEmpty++;
            if (stats.examples.length < 5) {
              stats.examples.push({
                buffetId: buffet.id,
                buffetName: buffet.name || 'N/A',
                rawValue: value,
                parsedValue: value
              });
            }
          }
        }
      }
    });

    // Display results
    console.log('='.repeat(80));
    console.log('placesTags Field Analysis');
    console.log('='.repeat(80));
    console.log(`Total records: ${allBuffets.length}`);
    console.log(`Field appears in records: ${stats.appearsInRecords}`);
    console.log(`Non-null values: ${stats.nonNull}`);
    console.log(`Non-empty values: ${stats.nonEmpty}`);
    console.log(`Percentage: ${((stats.nonEmpty / allBuffets.length) * 100).toFixed(2)}%`);
    console.log(`Unique tags: ${stats.uniqueValues.size}`);
    
    if (stats.uniqueValues.size > 0 && stats.uniqueValues.size <= 30) {
      console.log(`Unique tag list: ${Array.from(stats.uniqueValues).join(', ')}`);
    } else if (stats.uniqueValues.size > 30) {
      console.log(`Unique tag list (first 30): ${Array.from(stats.uniqueValues).slice(0, 30).join(', ')}...`);
    }
    
    if (stats.examples.length > 0) {
      console.log(`\nExample values (${stats.examples.length} examples):`);
      stats.examples.forEach((example, index) => {
        console.log(`\n  Example ${index + 1}:`);
        console.log(`    Buffet ID: ${example.buffetId}`);
        console.log(`    Buffet Name: ${example.buffetName}`);
        if (example.parseError) {
          console.log(`    Parse error: ${example.parseError}`);
          console.log(`    Raw value: ${JSON.stringify(example.rawValue).substring(0, 200)}${JSON.stringify(example.rawValue).length > 200 ? '...' : ''}`);
        } else if (example.parsedValue !== null) {
          const valueStr = JSON.stringify(example.parsedValue);
          console.log(`    Parsed value: ${valueStr.substring(0, 300)}${valueStr.length > 300 ? '...' : ''}`);
        } else {
          console.log(`    Raw value: ${example.rawValue.substring(0, 200)}${example.rawValue.length > 200 ? '...' : ''}`);
        }
      });
    } else {
      console.log('\n  No non-empty values found for placesTags');
    }

    // Summary
    console.log('\n' + '='.repeat(80));
    console.log('SUMMARY');
    console.log('='.repeat(80));
    if (stats.nonEmpty === 0) {
      console.log('❌ No values found - field is empty');
    } else {
      console.log(`✓ Found ${stats.nonEmpty} non-empty value(s)`);
      console.log(`✓ Found ${stats.uniqueValues.size} unique tag(s)`);
    }

  } catch (error) {
    console.error('Error:', error);
    throw error;
  }
}

checkPlacesTags()
  .then(() => {
    console.log('\n✓ Analysis complete!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('✗ Error:', error);
    process.exit(1);
  });

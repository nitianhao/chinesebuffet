// Script to migrate genderNeutralRestrooms from buffets table to structuredData table
// Run with: node scripts/migrate-gender-neutral-restrooms.js [--all]
...
async function migrateGenderNeutralRestrooms() {
  console.log('='.repeat(80));
  console.log('Migrating genderNeutralRestrooms to structuredData table');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Filter buffets that have genderNeutralRestrooms
    const buffets = allBuffets.buffets.filter(b => 
      b.genderNeutralRestrooms !== null && b.genderNeutralRestrooms !== undefined
    );

    if (buffets.length === 0) {
      console.log('✓ No buffets found with genderNeutralRestrooms');
      return;
    }
    ...
    console.log(`✓ Found ${buffets.length} buffet(s) with genderNeutralRestrooms`);
      ...
      try {
        ...
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {
              $: { where: { type: 'genderNeutralRestrooms' } }
            }
          }
        };
        ...
        const transaction = db.tx.structuredData[structuredDataId]
          .create({
            data: JSON.stringify(buffet.genderNeutralRestrooms),
            type: 'genderNeutralRestrooms',
            createdAt: now,
            updatedAt: now,
          })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: genderNeutralRestrooms)`);
        console.log(`  → Value: ${buffet.genderNeutralRestrooms}\n`);
    ...
    if (!migrateAll && migrated > 0) {
      ...
      console.log('  To migrate all records, run: node scripts/migrate-gender-neutral-restrooms.js --all\n');
    }
  } catch (error) {

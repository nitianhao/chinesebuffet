/**
 * Generate SEO-optimized "Nearby Communications & Technology" descriptions for buffets
 * 
 * Generates HTML descriptions based on POI data from the database.
 * 
 * Example commands:
 *   npx tsx scripts/generate-communications-technology.ts --limit 10
 *   npx tsx scripts/generate-communications-technology.ts --limit 10 --write
 *   npx tsx scripts/generate-communications-technology.ts --write --resume --concurrency 3
 */
...
const CHECKPOINT_FILE = path.join(__dirname, 'checkpoint-communications-technology.json');
...
function isMobileTelecomProvider(category: string, name: string): boolean {
  const catLower = category.toLowerCase();
  const nameLower = name.toLowerCase();
  
  // Check category
  const mobileCategories = ['mobile_phone', 'telecom', 'communications', 'wireless'];
  if (mobileCategories.some(cat => catLower.includes(cat))) {
    return true;
  }
  
  // Check name for common mobile/telecom brands
  const mobileBrands = ['t-mobile', 'verizon', 'at&t', 'sprint', 'cricket', 'boost', 'metro', 'straight talk', 'mint', 'visible'];
  if (mobileBrands.some(brand => nameLower.includes(brand))) {
    return true;
  }
...
function determineSummaryNoun(pois: CleanPoiItem[]): string {
  if (pois.length === 0) {
    return 'communications and technology services';
  }
  
  const mobileTelecomCount = pois.filter(poi => 
    isMobileTelecomProvider(poi.category, poi.name)
  ).length;
  
  const percentage = (mobileTelecomCount / pois.length) * 100;
  
  if (percentage >= 70) {
    return 'mobile and wireless service providers';
  }
...
function buildPrompt(
  ...
  return `Generate HTML for nearby communications & technology services section.
...
OUTPUT STRUCTURE:
Output a single section for Communications & Technology services:

<h3>Communications & Technology Services Nearby</h3>
...
- End with a period.
- Do NOT include category labels unless the category is particularly relevant (e.g., "phone repair", "internet service").
...
- Example correct format: "T-Mobile — ~0.5 mi — 3405 Commercial Street Southeast."
- Example correct format (no address): "Verizon — ~460 ft."
...
function validateHTML(
  ...
  const hasSection = html.includes('Communications & Technology Services Nearby');
...
async function processBuffet(
  ...
  // Skip if communicationsTechnology already exists (DO NOT call LLM)
  if (buffet.communicationsTechnology && buffet.communicationsTechnology.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  try {
    ...
    if (options.write) {
      await db.transact([db.tx.buffets[buffetId].update({ communicationsTechnology: html })]);
    }
...
async function main() {
  ...
  console.log('='.repeat(80));
  console.log('SEO Communications & Technology Description Generator');
  console.log('='.repeat(80));
  ...
  while (processed < limit) {
    ...
    for (const buffet of buffets) {
      ...
      // Skip if already has communicationsTechnology
      if (buffet.communicationsTechnology && buffet.communicationsTechnology.trim().length > 0) {

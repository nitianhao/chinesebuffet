/**
 * Generate SEO-optimized "Utilities & Infrastructure" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.utilitiesInfrastructure (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-utilities-infrastructure.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-utilities-infrastructure.ts --concurrency 5
 *   npx tsx scripts/generate-poi-utilities-infrastructure.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-utilities-infrastructure.ts --resume
 *   npx tsx scripts/generate-poi-utilities-infrastructure.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Utilities & Infrastructure';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'utilities-infrastructure.checkpoint.json');
...
type BuffetRecord = {
  id: string;
  name?: string | null;
  utilitiesInfrastructure?: string | null;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    // Other utilities
    'vending_machine': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'change_machine': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'ticket_validator': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'turn_stile': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'weighbridge': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'trolley_bay': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
    'street_lamp': { bucketKey: 'other', bucketLabel: 'Other utilities', typePhrase: 'utility services' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other utilities & infrastructure', typePhrase: 'utilities & infrastructure' };
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Utilities & Infrastructure
...
/**
 * Update names for bucket context (no special handling needed for Utilities & Infrastructure)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Utilities & Infrastructure
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'utilities & infrastructure';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'utilities & infrastructure';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 utility & infrastructure service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} utilities & infrastructure services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Utilities & Infrastructure
...
async function processBuffet(
  ...
  // Skip if utilitiesInfrastructure already exists (unless force)
  if (!options.force && buffet.utilitiesInfrastructure && buffet.utilitiesInfrastructure.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ utilitiesInfrastructure: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (utilitiesInfrastructure already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Utilities & Infrastructure Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.utilitiesInfrastructure`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has utilitiesInfrastructure (unless force)
        if (!force && buffet.utilitiesInfrastructure && buffet.utilitiesInfrastructure.trim().length > 0) {

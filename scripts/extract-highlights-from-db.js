// Script to extract "Highlights" from additionalInfo field in database
// and populate the highlights field
// Run with: node scripts/extract-highlights-from-db.js [--all]
...
/**
 * Extract "Highlights" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of highlights, or null if not found
 */
function extractHighlights(additionalInfo) {
  if (!additionalInfo) {
    return null;
  }
  ...
  // Check if "Highlights" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const highlightsKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'highlights' || key === 'Highlights'
  );
  ...
  const highlightsArray = infoObj[highlightsKey];

  if (!Array.isArray(highlightsArray) || highlightsArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "Fast service": true }, { "Great cocktails": true }]
  const highlightsItems = [];
  
  for (const item of highlightsArray) {
    if (typeof item === 'object' && item !== null) {
      ...
      for (const key of keys) {
        if (item[key] === true || item[key]) {
          highlightsItems.push(key);
        }
      }
    } else if (typeof item === 'string') {
      // If it's already a string, just add it
      highlightsItems.push(item);
    }
  }
  ...
  return highlightsItems.join(', ');
}

async function extractHighlightsFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Highlights" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Highlights"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithHighlights = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const highlights = extractHighlights(additionalInfo);
        ...
        const currentHighlights = buffet.highlights || null;
        if (currentHighlights === highlights) {
    ...
    console.log(`\n✓ Found ${foundWithHighlights} records with "Highlights" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct highlights value or have no "Highlights" data.\n');
      return;
    }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → highlights: ${update.oldValue || 'null'} → "${update.highlights}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Highlights": ${foundWithHighlights}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-highlights-from-db.js --all\n');
    }
...
extractHighlightsFromDB().catch(error => {

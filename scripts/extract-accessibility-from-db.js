// Script to extract "Accessibility" from additionalInfo field in database
// and populate the accessibility field
// Run with: node scripts/extract-accessibility-from-db.js [--all]
...
/**
 * Extract "Accessibility" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of accessibility features, or null if not found
 */
function extractAccessibility(additionalInfo) {
  if (!additionalInfo) {
    return null;
  }
  ...
  // Check if "Accessibility" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const accessibilityKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'accessibility' || key === 'Accessibility'
  );
  ...
  const accessibilityArray = infoObj[accessibilityKey];

  if (!Array.isArray(accessibilityArray) || accessibilityArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "Wheelchair accessible restroom": true }, { "Wheelchair accessible seating": true }]
  const accessibilityItems = [];
  
  for (const item of accessibilityArray) {
  ...
  return accessibilityItems.join(', ');
}

async function extractAccessibilityFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Accessibility" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Accessibility"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithAccessibility = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const accessibility = extractAccessibility(additionalInfo);
        ...
        foundWithAccessibility++;

        // Check if it's already set and different
        const currentAccessibility = buffet.accessibility || null;
        if (currentAccessibility === accessibility) {
    ...
    console.log(`\n✓ Found ${foundWithAccessibility} records with "Accessibility" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct accessibility value or have no "Accessibility" data.\n');
      return;
    }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → accessibility: ${update.oldValue || 'null'} → "${update.accessibility}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Accessibility": ${foundWithAccessibility}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-accessibility-from-db.js --all\n');
    }
...
extractAccessibilityFromDB().catch(error => {

// Script to migrate businessAcceptsCreditCards from buffets table to structuredData table
// Run with: node scripts/migrate-business-accepts-credit-cards.js [--all]
...
async function migrateBusinessAcceptsCreditCards() {
  console.log('='.repeat(80));
  console.log('Migrating businessAcceptsCreditCards to structuredData table');
  console.log('='.repeat(80));
  console.log(`Mode: ${migrateAll ? 'ALL records' : 'TEST (1 record only)'}\n`);

  try {
    // Query buffets - we'll filter for those with businessAcceptsCreditCards in JavaScript
    console.log('Querying buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    // Fetch buffets in batches and filter for those with businessAcceptsCreditCards
    while (true) {
      const query = {
        buffets: {
          $: {
      ...
      // Filter for buffets that have businessAcceptsCreditCards set (not null/undefined)
      const buffetsWithData = batch.filter(b => 
        b.businessAcceptsCreditCards !== null && 
        b.businessAcceptsCreditCards !== undefined
      );
    ...
    if (buffets.length === 0) {
      console.log('✓ No buffets found with businessAcceptsCreditCards');
      return;
    }

    console.log(`✓ Found ${buffets.length} buffet(s) with businessAcceptsCreditCards\n`);
      ...
      try {
        ...
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {
              $: { where: { type: 'businessAcceptsCreditCards' } }
            }
          }
        };
        ...
        // Get the businessAcceptsCreditCards value
        const businessAcceptsCreditCardsValue = buffet.businessAcceptsCreditCards;
        ...
        const structuredDataRecord = {
          data: JSON.stringify({ businessAcceptsCreditCards: businessAcceptsCreditCardsValue }),
          type: 'businessAcceptsCreditCards',
          createdAt: now,
          updatedAt: now,
        };
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: businessAcceptsCreditCards, value: ${businessAcceptsCreditCardsValue})\n`);
        ...
        if (!migrateAll && migrated === 1) {
          ...
          console.log('  node scripts/migrate-business-accepts-credit-cards.js --all\n');
          break;
        }
...
migrateBusinessAcceptsCreditCards().catch(error => {

// Script to check bookingLinks field
...
async function checkBookingLinks() {
  console.log('Checking bookingLinks field...\n');
  ...
  try {
    ...
    allBuffets.forEach(buffet => {
      if ('bookingLinks' in buffet) {
        stats.appearsInRecords++;
        if (buffet.bookingLinks !== null && buffet.bookingLinks !== undefined) {
          stats.nonNull++;
          const value = buffet.bookingLinks;
          let isEmpty = false;
    ...
    console.log('bookingLinks Field Analysis');
    console.log('='.repeat(80));
    ...
    if (stats.examples.length > 0) {
      ...
      console.log('\n  No non-empty values found for bookingLinks');
    }

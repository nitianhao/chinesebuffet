/**
 * Generate SEO-optimized "Sports & Fitness" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.sportsFitness (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-sports-fitness.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-sports-fitness.ts --concurrency 5
 *   npx tsx scripts/generate-poi-sports-fitness.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-sports-fitness.ts --resume
 *   npx tsx scripts/generate-poi-sports-fitness.ts --force --limit 100
 */
...
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'sports-fitness.checkpoint.json');
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    ...
    'fitness_centre': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    'fitness': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    'fitness_station': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    'gym': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    'personal_trainer': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    'personal_training': { bucketKey: 'gyms', bucketLabel: 'Gyms & fitness centers', typePhrase: 'gyms and fitness centers' },
    
    // Yoga & pilates
    'yoga': { bucketKey: 'yoga', bucketLabel: 'Yoga & pilates studios', typePhrase: 'yoga and pilates studios' },
    'pilates': { bucketKey: 'yoga', bucketLabel: 'Yoga & pilates studios', typePhrase: 'yoga and pilates studios' },
    
    // Sports facilities
    'sports_centre': { bucketKey: 'sports', bucketLabel: 'Sports facilities', typePhrase: 'sports facilities' },
    'sports_hall': { bucketKey: 'sports', bucketLabel: 'Sports facilities', typePhrase: 'sports facilities' },
    'sports': { bucketKey: 'sports', bucketLabel: 'Sports facilities', typePhrase: 'sports facilities' },
    'stadium': { bucketKey: 'sports', bucketLabel: 'Sports facilities', typePhrase: 'sports facilities' },
    
    // Golf courses
    'golf': { bucketKey: 'golf', bucketLabel: 'Golf courses', typePhrase: 'golf courses' },
    'golf_course': { bucketKey: 'golf', bucketLabel: 'Golf courses', typePhrase: 'golf courses' },
    
    // Martial arts
    'martial_arts': { bucketKey: 'martial_arts', bucketLabel: 'Martial arts studios', typePhrase: 'martial arts studios' },
    'dojo': { bucketKey: 'martial_arts', bucketLabel: 'Martial arts studios', typePhrase: 'martial arts studios' },
  };
  
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other sports & fitness venues', typePhrase: 'sports & fitness venues' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'dojo': 'Martial arts dojo',
  };
  
  return fallbacks[cat] || 'Sports & fitness venue';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Sports & Fitness
...
/**
 * Update names for bucket context (no special handling needed for Sports & Fitness)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Sports & Fitness
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'sports & fitness venues';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'sports & fitness venues';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 sports & fitness venue is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} sports & fitness venues are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Sports & Fitness
 ...
 */
function validateOutput(output: SportsFitnessOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: SportsFitnessOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if sportsFitness already exists (unless force)
  if (!options.force && buffet.sportsFitness && buffet.sportsFitness.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: SportsFitnessOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ sportsFitness: jsonString })]);
  }
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Sports & Fitness Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.sportsFitness`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has sportsFitness (unless force)
        if (!force && buffet.sportsFitness && buffet.sportsFitness.trim().length > 0) {

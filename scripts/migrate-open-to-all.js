// Script to migrate openToAll from buffets table to structuredData table
// Run with: node scripts/migrate-open-to-all.js [--all]
...
async function migrateOpenToAll() {
  try {
    console.log('Starting migration of openToAll to structuredData...\n');
    ...
    // Filter buffets that have openToAll set
    const buffets = allBuffets.buffets.filter(b => b.openToAll !== null && b.openToAll !== undefined);

    if (buffets.length === 0) {
      console.log('No buffets found with openToAll field set.');
      return;
    }

    console.log(`Found ${buffets.length} buffets with openToAll field set.\n`);
    ...
    if (!migrateAll) {
      ...
      console.log('To migrate all records, run: node scripts/migrate-open-to-all.js --all\n');
    }
      ...
      try {
        ...
        const batchTransactions = [
          db.tx.structuredData[structuredDataId]
            .create({
              data: JSON.stringify(buffet.openToAll),
              type: 'openToAll',
              createdAt: now,
              updatedAt: now,
            })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: openToAll)`);
        console.log(`  → Value: ${buffet.openToAll}\n`);
    ...
    if (!migrateAll && migrated > 0) {
      ...
      console.log('  node scripts/migrate-open-to-all.js --all');
    }
  } catch (error) {

// Script to migrate acceptsGooglePay from buffets table to structuredData table
// Run with: node scripts/migrate-accepts-google-pay.js [--all]
...
async function migrateAcceptsGooglePay() {
  console.log('='.repeat(80));
  console.log('Migrating acceptsGooglePay to structuredData table');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Filter buffets that have acceptsGooglePay
    const buffets = allBuffets.buffets.filter(b => 
      b.acceptsGooglePay !== null && b.acceptsGooglePay !== undefined
    );

    if (buffets.length === 0) {
      console.log('✓ No buffets found with acceptsGooglePay');
      return;
    }
    ...
    console.log(`✓ Found ${buffets.length} buffet(s) with acceptsGooglePay`);
    ...
    for (const buffet of buffetsToProcess) {
      ...
      try {
        ...
        const transaction = db.tx.structuredData[structuredDataId]
          .create({
            data: JSON.stringify(buffet.acceptsGooglePay),
            type: 'acceptsGooglePay',
            createdAt: now,
            updatedAt: now,
          })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: acceptsGooglePay)`);
        console.log(`  → Value: ${buffet.acceptsGooglePay}\n`);
    ...
    if (!migrateAll && migrated > 0) {
      ...
      console.log('  To migrate all records, run: node scripts/migrate-accepts-google-pay.js --all\n');
    }
  } catch (error) {

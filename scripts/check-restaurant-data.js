// Script to check restaurantData field
...
async function checkRestaurantData() {
  console.log('Checking restaurantData field...\n');
  ...
  try {
    ...
    allBuffets.forEach(buffet => {
      if ('restaurantData' in buffet) {
        stats.appearsInRecords++;
        if (buffet.restaurantData !== null && buffet.restaurantData !== undefined) {
          stats.nonNull++;
          const value = buffet.restaurantData;
          let isEmpty = false;
    ...
    console.log('restaurantData Field Analysis');
    console.log('='.repeat(80));
    ...
    if (stats.examples.length > 0) {
      ...
      console.log('\n  No non-empty values found for restaurantData');
    }

// Script to check if menu2 URLs are valid
const fs = require('fs');
...
async function checkMenu2Urls() {
  // Filter records that don't have menu2_check yet but have menu2
  const recordsToCheck = data.filter(r => !r.menu2_check && r.menu2);
  const totalRecords = recordsToCheck.length;
  const startTime = Date.now();
  
  console.log(`Checking ${totalRecords} menu2 URLs...`);
  console.log(`(Skipping ${data.length - totalRecords} records that already have menu2_check or no menu2 URL)`);

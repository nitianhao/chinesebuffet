// Script to check rank and reserveTableUrl fields
...
const fieldsToCheck = ['rank', 'reserveTableUrl'];
...
async function checkFields() {
  ...
  try {
      ...
      fieldsToCheck.forEach(field => {
        if (field in buffet) {
          ...
          if (buffet[field] !== null && buffet[field] !== undefined) {
            ...
            } else if (field === 'reserveTableUrl') {
              // For reserveTableUrl, it's a string
    ...
    fieldsToCheck.forEach(field => {
      ...
      if (field === 'rank' && stats.minValue !== null) {
        ...
        console.log(`Unique URLs: ${stats.uniqueValues.size}`);
      }
      ...
      if (stats.examples.length > 0) {
        ...
        stats.examples.forEach((example, index) => {
          ...
          if (field === 'rank') {
            console.log(`    rank: ${example.value} (type: ${typeof example.value})`);
          } else {
            console.log(`    reserveTableUrl: ${example.value.substring(0, 150)}${example.value.length > 150 ? '...' : ''}`);
          }
        });
      } else {

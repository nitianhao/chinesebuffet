// Script to migrate businessAcceptsApplePay from buffets table to structuredData table
// Run with: node scripts/migrate-business-accepts-apple-pay.js [--all]
...
async function migrateBusinessAcceptsApplePay() {
  console.log('='.repeat(80));
  console.log('Migrating businessAcceptsApplePay to structuredData table');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Filter buffets that have businessAcceptsApplePay
    const buffets = allBuffets.buffets.filter(b => 
      b.businessAcceptsApplePay !== null && b.businessAcceptsApplePay !== undefined
    );

    if (buffets.length === 0) {
      console.log('✓ No buffets found with businessAcceptsApplePay');
      return;
    }
    ...
    console.log(`✓ Found ${buffets.length} buffet(s) with businessAcceptsApplePay`);
      ...
      try {
        ...
        const existingQuery = {
          buffets: {
            $: { where: { id: buffet.id } },
            structuredData: {
              $: { where: { type: 'businessAcceptsApplePay' } }
            }
          }
        };
        ...
        const transaction = db.tx.structuredData[structuredDataId]
          .create({
            data: JSON.stringify(buffet.businessAcceptsApplePay),
            type: 'businessAcceptsApplePay',
            createdAt: now,
            updatedAt: now,
          })
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: businessAcceptsApplePay)`);
        console.log(`  → Value: ${buffet.businessAcceptsApplePay}\n`);
    ...
    if (!migrateAll && migrated > 0) {
      ...
      console.log('  To migrate all records, run: node scripts/migrate-business-accepts-apple-pay.js --all\n');
    }
  } catch (error) {
...
migrateBusinessAcceptsApplePay();

// Script to check restaurantData and searchPageUrl fields
...
const fieldsToCheck = ['restaurantData', 'searchPageUrl'];
          ...
          if (buffet[field] !== null && buffet[field] !== undefined) {
            ...
            if (typeof value === 'string') {
              ...
              if (trimmed !== '' && trimmed !== 'null' && trimmed !== '[]' && trimmed !== '{}') {
                // For JSON strings (restaurantData), try to parse and check if it's meaningful
                if (field === 'restaurantData' && (trimmed.startsWith('{') || trimmed.startsWith('['))) {
                  try {
                  ...
                  // Non-JSON string with content (searchPageUrl or non-JSON restaurantData)
                  isEmpty = false;
                  fieldStats[field].nonEmpty++;
                  fieldStats[field].uniqueValues.add(trimmed);
                  if (fieldStats[field].examples.length < 5) {

// Script to check gasPrices field
...
async function checkGasPrices() {
  console.log('Checking gasPrices field...\n');
  ...
  try {
    ...
    allBuffets.forEach(buffet => {
      if ('gasPrices' in buffet) {
        stats.appearsInRecords++;
        if (buffet.gasPrices !== null && buffet.gasPrices !== undefined) {
          stats.nonNull++;
          const value = buffet.gasPrices;
          let isEmpty = false;
    ...
    console.log('gasPrices Field Analysis');
    console.log('='.repeat(80));
    ...
    if (stats.examples.length > 0) {
      ...
      console.log('\n  No non-empty values found for gasPrices');
    }

/**
 * Generate SEO-optimized "Government & Public Services" descriptions for buffets
 * 
 ...
 * 
 * Output stored in: buffets.governmentPublicServices (JSON string)
 * 
 * Example commands:
 *   npx tsx scripts/generate-poi-government-public-services.ts --dry-run --limit 5
 *   npx tsx scripts/generate-poi-government-public-services.ts --concurrency 5
 *   npx tsx scripts/generate-poi-government-public-services.ts --buffetId <id> --dry-run
 *   npx tsx scripts/generate-poi-government-public-services.ts --resume
 *   npx tsx scripts/generate-poi-government-public-services.ts --force --limit 100
 */
...
const TARGET_GROUP = 'Government & Public Services';
const MAX_ITEMS_PER_BUCKET = 6;
const MAX_TOTAL_ITEMS = 12;
const CHECKPOINT_DIR = path.join(__dirname, 'checkpoints');
const CHECKPOINT_FILE = path.join(CHECKPOINT_DIR, 'government-public-services.checkpoint.json');
...
type BuffetRecord = {
  id: string;
  name?: string | null;
  governmentPublicServices?: string | null;
...
type GovernmentPublicServicesOutput = {
  summary: string;
...
function bucketize(category: string | null | undefined): { bucketKey: string; bucketLabel: string; typePhrase: string } {
  ...
  const bucketMap: Record<string, { bucketKey: string; bucketLabel: string; typePhrase: string }> = {
    // Government offices
    'government': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    'townhall': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    'courthouse': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    'public_building': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    'polling_station': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    'diplomatic': { bucketKey: 'government', bucketLabel: 'Government offices', typePhrase: 'government offices' },
    
    // Emergency services
    'police': { bucketKey: 'emergency', bucketLabel: 'Emergency services', typePhrase: 'emergency services' },
    'fire_station': { bucketKey: 'emergency', bucketLabel: 'Emergency services', typePhrase: 'emergency services' },
    
    // Postal services
    'post_office': { bucketKey: 'postal', bucketLabel: 'Postal services', typePhrase: 'postal services' },
    'post_box': { bucketKey: 'postal', bucketLabel: 'Postal services', typePhrase: 'postal services' },
    'post_depot': { bucketKey: 'postal', bucketLabel: 'Postal services', typePhrase: 'postal services' },
    'letter_box': { bucketKey: 'postal', bucketLabel: 'Postal services', typePhrase: 'postal services' },
    
    // Public facilities
    'library': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'community_centre': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'social_centre': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'social_facility': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'toilets': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    'drinking_water': { bucketKey: 'facilities', bucketLabel: 'Public facilities', typePhrase: 'public facilities' },
    
    // Military & security
    'military': { bucketKey: 'military', bucketLabel: 'Military & security', typePhrase: 'military and security facilities' },
    'military_recruitment': { bucketKey: 'military', bucketLabel: 'Military & security', typePhrase: 'military and security facilities' },
    'border_control': { bucketKey: 'military', bucketLabel: 'Military & security', typePhrase: 'military and security facilities' },
    'ranger_station': { bucketKey: 'military', bucketLabel: 'Military & security', typePhrase: 'military and security facilities' },
    ...
    'grit_bin': { bucketKey: 'waste', bucketLabel: 'Waste management', typePhrase: 'waste management facilities' },
  ...
  return bucketMap[cat] || { bucketKey: 'other', bucketLabel: 'Other government & public services', typePhrase: 'government & public services' };
}
...
function getCategoryFallback(category: string | null | undefined, tagsObj: Record<string, any>): string {
  ...
  const fallbacks: Record<string, string> = {
    ...
    'diplomatic': 'Diplomatic facility',
  ...
  return fallbacks[cat] || 'Government & public service';
}
...
function dedupePois(pois: PoiRecord[]): { deduped: PoiRecord[]; removedCount: number } {
  ...
  // No special category-specific dedupe needed for Government & Public Services
...
/**
 * Update names for bucket context (no special handling needed for Government & Public Services)
 */
function updateNamesForBucketContext(items: CleanPoi[]): CleanPoi[] {
  // No special name updates needed for Government & Public Services
  return items;
}
...
function getBucketTypesSentence(
  ...
  const typeA = bucketTypePhrases[bucketKeys[0]] || 'government & public services';
  const typeB = bucketTypePhrases[bucketKeys[1]] || 'government & public services';
...
function buildSummary(extras: ProcessExtras, trimmedPois: CleanPoi[], buffetId: string): string {
  ...
  if (poiCount === 1) {
    const sentence1 = `1 government & public service is listed ${closestSummaryDistanceText} from the buffet.`;
  ...
  // Multiple POIs
  const sentence1 = `${poiCount} government & public services are listed ${rangeText}, with the closest at ${closestSummaryDistanceText}.`;
  ...
  // No intent hooks for Government & Public Services
 ...
 */
function validateOutput(output: GovernmentPublicServicesOutput, extras: ProcessExtras): { valid: boolean; error?: string } {
  // Check required keys
...
async function processBuffet(
...
): Promise<{ status: string; output?: GovernmentPublicServicesOutput; poiCount?: number; removedCount?: number }> {
  ...
  // Skip if governmentPublicServices already exists (unless force)
  if (!options.force && buffet.governmentPublicServices && buffet.governmentPublicServices.trim().length > 0) {
    checkpoint[buffetId] = {
  ...
  // Assemble output
  const output: GovernmentPublicServicesOutput = {
    summary,
    highlights,
  ...
  if (!options.dryRun) {
    ...
    await db.transact([db.tx.buffets[buffetId].update({ governmentPublicServices: jsonString })]);
  }
...
function printBuffetOutput(
  ...
  result: { 
    status: string; 
    output?: GovernmentPublicServicesOutput;
    poiCount?: number;
    removedCount?: number;
  }
) {
  ...
  if (result.status === 'generated') {
    ...
    console.log(`STATUS: SKIPPED_EXISTING (governmentPublicServices already present)`);
...
async function main() {
  ...
  // Print header
  console.log('='.repeat(80));
  console.log('SEO Government & Public Services Description Generator (Deterministic)');
  console.log('='.repeat(80));
  console.log(`Target group: ${TARGET_GROUP}`);
  console.log(`Output field: buffets.governmentPublicServices`);
  ...
  if (buffetId) {
    ...
    while (true) {
      ...
      for (const buffet of buffets) {
        ...
        // Skip if already has governmentPublicServices (unless force)
        if (!force && buffet.governmentPublicServices && buffet.governmentPublicServices.trim().length > 0) {

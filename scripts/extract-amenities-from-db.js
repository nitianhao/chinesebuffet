// Script to extract "Amenities" from additionalInfo field in database
// and populate the amenities field
// Run with: node scripts/extract-amenities-from-db.js [--all]
...
/**
 * Extract "Amenities" values from additionalInfo
 * @param {string|object} additionalInfo - The additionalInfo JSON string or object
 * @returns {string|null} - Comma-separated list of amenities, or null if not found
 */
function extractAmenities(additionalInfo) {
  ...
  // Check if "Amenities" exists in the object
  if (!infoObj || typeof infoObj !== 'object') {
    return null;
  }

  // Try different case variations of the key
  const amenitiesKey = Object.keys(infoObj).find(
    key => key.toLowerCase() === 'amenities' || key === 'Amenities'
  );
  ...
  const amenitiesArray = infoObj[amenitiesKey];

  if (!Array.isArray(amenitiesArray) || amenitiesArray.length === 0) {
    return null;
  }

  // Extract keys from objects like [{ "Restroom": true }, { "Wi-Fi": true }]
    ...
    if (typeof item === 'object' && item !== null) {
      ...
      for (const key of keys) {
        if (item[key] === true || item[key]) {
          amenitiesItems.push(key);
        }
      }
    } else if (typeof item === 'string') {
      // If it's already a string, just add it
      amenitiesItems.push(item);
    }
  }
  ...
  return amenitiesItems.join(', ');
}

async function extractAmenitiesFromDB() {
  console.log('='.repeat(80));
  console.log('Extracting "Amenities" from additionalInfo field in database');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Process each buffet and extract "Amenities"
    let updated = 0;
    let skipped = 0;
    let errors = 0;
    let foundWithAmenities = 0;
    const updates = [];

    for (const buffet of recordsToProcess) {
      try {
        const additionalInfo = buffet.additionalInfo;
        const amenities = extractAmenities(additionalInfo);
        ...
        const currentAmenities = buffet.amenities || null;
        if (currentAmenities === amenities) {
    ...
    console.log(`\n✓ Found ${foundWithAmenities} records with "Amenities" data`);
    console.log(`✓ Prepared ${updates.length} updates. Applying to database...\n`);

    if (updates.length === 0) {
      console.log('No updates to apply. All records already have the correct amenities value or have no "Amenities" data.\n');
      return;
    }
    ...
    for (let i = 0; i < updates.length; i += updateBatchSize) {
      ...
      try {
        ...
        for (const update of batch) {
          ...
          console.log(`  → amenities: ${update.oldValue || 'null'} → "${update.amenities}"`);
          updated++;
        }
    ...
    console.log(`  📊 Found with "Amenities": ${foundWithAmenities}`);
    console.log('='.repeat(80));

    if (!migrateAll && updated > 0) {
      console.log('\n✓ Test extraction completed successfully!');
      console.log('To process all records, run:');
      console.log('  node scripts/extract-amenities-from-db.js --all\n');
    }
...
extractAmenitiesFromDB().catch(error => {

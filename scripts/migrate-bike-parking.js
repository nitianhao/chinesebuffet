// Script to migrate bikeParking from buffets table to structuredData table
// Run with: node scripts/migrate-bike-parking.js [--all]
...
async function migrateBikeParking() {
  console.log('='.repeat(80));
  console.log('Migrating bikeParking to structuredData table');
  console.log('='.repeat(80));
  console.log(`Mode: ${migrateAll ? 'ALL records' : 'TEST (1 record only)'}\n`);

  try {
    // Query buffets - we'll filter for those with bikeParking in JavaScript
    console.log('Querying buffets...');
    let allBuffets = [];
    let offset = 0;
    const limit = 1000;
    
    // Fetch buffets in batches and filter for those with bikeParking
    while (true) {
      const query = {
        buffets: {
          $: {
      ...
      // Filter for buffets that have bikeParking set (not null/undefined)
      const buffetsWithData = batch.filter(b => 
        b.bikeParking !== null && 
        b.bikeParking !== undefined
      );
    ...
    if (buffets.length === 0) {
      console.log('✓ No buffets found with bikeParking');
      return;
    }

    console.log(`✓ Found ${buffets.length} buffet(s) with bikeParking\n`);
    ...
    for (const buffet of buffets) {
      try {
        ...
        // Get the bikeParking value
        const bikeParkingValue = buffet.bikeParking;
        ...
        const structuredDataRecord = {
          data: JSON.stringify({ bikeParking: bikeParkingValue }),
          type: 'bikeParking',
          createdAt: now,
          updatedAt: now,
        };
        ...
        console.log(`  → Created structuredData record (id: ${structuredDataId}, type: bikeParking, value: ${bikeParkingValue})\n`);
        ...
        if (!migrateAll && migrated === 1) {
          ...
          console.log('  node scripts/migrate-bike-parking.js --all\n');
          break;
        }
...
migrateBikeParking().catch(error => {

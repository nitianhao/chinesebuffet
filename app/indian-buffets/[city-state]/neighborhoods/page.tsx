...
import { getCityNeighborhoodsRollup, STATE_ABBR_TO_NAME } from '@/lib/indian-rollups';
...
export async function generateMetadata({ params }: NeighborhoodsIndexPageProps): Promise<Metadata> {
  ...
  const selfCanonical = `${BASE_URL}/indian-buffets/${citySlug}/neighborhoods`;
  if (!data) {
    return {
  ...
  return {
    title: `Indian Buffets by Neighborhood in ${data.cityName}, ${data.stateAbbr}`,
    description: `Browse Indian buffets in ${neighborhoodCount} neighborhoods across ${data.cityName}, ${data.stateAbbr}. Find all-you-can-eat restaurants with hours, prices, and reviews.`,
    alternates: {
      ...
      <header className="bg-[var(--surface)] shadow-sm border-b border-[var(--border)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="text-4xl font-bold text-[var(--text)] mb-2">
            Indian Buffets by Neighborhood in {cityName}
          </h1>
      ...
      <section className="bg-[var(--surface)] py-8 border-b border-[var(--border)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[var(--text-secondary)] max-w-3xl">
            {!isEmpty 
              ? `Explore Indian buffets in ${neighborhoods.length} neighborhoods across ${cityName}, ${stateAbbr}. 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-white/60">
              Indian Buffets Directory - Find all-you-can-eat Indian buffets in {cityName} neighborhoods
            </p>
          </div>
        </div>
      </footer>

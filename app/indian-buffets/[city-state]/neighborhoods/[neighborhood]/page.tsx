...
export async function generateMetadata({ params }: NeighborhoodPageProps): Promise<Metadata> {
  ...
  const pathname = `/indian-buffets/${citySlug}/neighborhoods/${neighborhoodSlug}`;
  ...
  if (!hasBuffets) {
    return {
      title: `Indian Buffets in ${data.neighborhoodName || neighborhoodSlug}`,
      description: `No Indian buffets currently listed in ${data.neighborhoodName || neighborhoodSlug}.`,
      robots,
      alternates: { canonical },
    };
  }
  ...
  return {
    title: `Top ${count} Indian Buffets in ${neighborhoodName}, ${cityName}`,
    description: `${count} top Indian buffets in ${neighborhoodName}, ${cityName}. All-you-can-eat, ratings & hours. Updated ${year}.`,
    alternates: { canonical },
    robots,
  };
}
...
export default async function NeighborhoodPage({ params }: NeighborhoodPageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: cityName, item: `${BASE_URL}/indian-buffets/${citySlug}` },
      {
        ...
        position: 4,
        name: 'Neighborhoods',
        item: `${BASE_URL}/indian-buffets/${citySlug}/neighborhoods`,
      },
      {
        '@type': 'ListItem',
        position: 5,
        name: neighborhoodName,
        item: `${BASE_URL}/indian-buffets/${citySlug}/neighborhoods/${neighborhoodSlug}`,
      },
    ],
  };
  ...
  const neighborhoodPageUrl = `${BASE_URL}/indian-buffets/${citySlug}/neighborhoods/${neighborhoodSlug}`;
  const itemListItems = buffets.slice(0, 50);
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    '@id': `${neighborhoodPageUrl}#itemlist`,
    name: `Indian Buffets in ${neighborhoodName}, ${cityName}`,
    ...
    <SiteShell>
        ...
        <div className="space-y-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-[var(--text)]">
              Indian Buffets in {neighborhoodName}, {cityName}, {state}
            </h1>
          ...
          <p className="text-sm text-[var(--text-secondary)] max-w-2xl">
            Find Indian buffet spots in {neighborhoodName} with ratings, reviews, and helpful details to plan your next meal.
          </p>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="prose max-w-none">
          <p className="text-[var(--text-secondary)] mb-4">
            Discover the best Indian buffets in {neighborhoodName}, {cityName}.
            With {buffetCount} {buffetCount === 1 ? 'location' : 'locations'} in this neighborhood,
            you're sure to find an all-you-can-eat Indian buffet nearby.
          </p>
          <p className="text-[var(--text-secondary)] mb-4">
            Whether you're looking for traditional Indian cuisine, Mongolian grill, sushi, or
            American-Indian favorites, our directory covers all the Indian buffet restaurants
            in {neighborhoodName}.
          </p>
          <p className="text-[var(--text-secondary)]">
            Each listing includes hours, prices, ratings, reviews, and detailed location information
            to help you find the perfect Indian buffet experience.
          </p>
        </div>
      </section>
      ...
      {false && hasFilter && (
        <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)] text-center">
          ...
          <Link
            href={`/indian-buffets/${citySlug}/neighborhoods/${neighborhoodSlug}`}
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-[var(--text)]">
            All Indian Buffets in {neighborhoodName}
          </h2>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <p className="text-sm text-[var(--text-secondary)] leading-relaxed max-w-3xl">
          Indian buffets in {neighborhoodName} give you a range of options for families, groups, and quick meals in
        ...
        <div className="mt-4 space-y-3">
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)]">
            ...
            <p className="px-4 pb-4 text-sm text-[var(--text-secondary)] leading-relaxed">
              There are {buffetCount} Indian buffets listed in {neighborhoodName}.
            </p>
          </details>
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-8">
        <p className="text-center text-sm text-[var(--muted)]">
          Indian Buffets Directory - Find all-you-can-eat Indian buffets in {neighborhoodName}, {cityName}
        </p>
      </section>
    </SiteShell>
  );

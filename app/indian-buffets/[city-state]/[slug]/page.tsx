...
export async function generateStaticParams() {
  const pairs = await getTopRatedBuffetSlugsForCuisine('Indian', 150);
...
function generateMetaDescription(buffet: any): string {
  ...
  parts.push(`${buffet.name} – Indian buffet${city}. Menu, prices, hours.`);
...
export async function generateMetadata({ params }: BuffetPageProps): Promise<Metadata> {
  ...
  const selfCanonical = `${baseUrl}/indian-buffets/${params['city-state']}/${params.slug}`;
  if (!buffet) {
  ...
  // only, so a Chinese buffet is reachable at this /indian-buffets/ URL and
  // would render as duplicate content titled "Indian Buffet". The page-level
  // notFound() guard does not currently take effect on this route, so noindex
  // here is what actually keeps the duplicate out of the index. See the mirror
  // of this block in the /chinese-buffets/ route.
  if (getCuisineBasePath(buffet.cuisineType) !== '/indian-buffets') {
    const robots = { index: false as const, follow: true as const };
    if (process.env.NODE_ENV === 'development') {
      console.log('[seo] buffet robots decided:', robots, `(cuisine mismatch: ${buffet.cuisineType} served under /indian-buffets)`);
    }
    return {
  ...
  const pagePath = `/indian-buffets/${params['city-state']}/${params.slug}`;
  ...
  const title = cityName
    ? `${buffet.name} – Indian Buffet in ${cityName} (Menu, Prices, Hours)`
    : `${buffet.name} – Indian Buffet (Menu, Prices, Hours)`;
  const metadata = {
    title,
    description,
    alternates: {
...
export default async function BuffetPage({ params }: BuffetPageProps) {
  ...
  // Cuisine guard: getBuffetNameBySlug() looks up on city+slug only, so a
  // Chinese buffet would otherwise render here under an /indian-buffets/ URL —
  // duplicate content with a mislabelled title. These cross-cuisine URLs have
  // never been indexed, so there is no link equity to preserve and 404 is the
  // correct signal. Note: redirect() does NOT survive this route's ISR
  // generation (revalidate=86400 + fetchCache='force-cache'), so notFound() is
  // used deliberately. Unknown/legacy cuisineType defaults to Chinese.
  if (getCuisineBasePath(buffet.cuisineType) !== '/indian-buffets') {
    notFound();
  }
  // Soft-deleted (delisted) buffets 301 to their city page rather than render a
  // thin listing that is excluded from every listing surface (orphan page).
  if (buffet.delisted) {
    endPage();
    permanentRedirect(`/indian-buffets/${params['city-state']}`);
  }
  const pagePath = `/indian-buffets/${params['city-state']}/${params.slug}`;
  const llmSummaryDraft = getLlmSeoSummaryDraft(pagePath);
  ...
  // Scope city peers to the SAME cuisine as this buffet, so nearby/same-city
  // links and counts point to Indian buffets (not the shared city's Chinese ones).
  const cityCuisineBuffets = (cityInfo?.buffets ?? []).filter(
    (b: any) => getCuisineNoun(b.cuisineType) === getCuisineNoun(buffet.cuisineType)
  );
  const cuisineBasePath = getCuisineBasePath(buffet.cuisineType);

  // Derive sameCityBuffets from same-cuisine city peers
  if (cityCuisineBuffets.length > 0) {
    sameCityBuffets = cityCuisineBuffets
  ...
  const cityData: { name: string; buffetCount: number } | null = cityInfo
    ? {
        name: cityInfo.city,
        buffetCount: cityCuisineBuffets.length || 0,
      }
    : null;
  ...
  if (stateAbbr && stateName) {
    breadcrumbItems.push({
      name: stateName,
      url: `/indian-buffets/states/${stateAbbr.toLowerCase()}`,
    });
  }
  ...
  if (cityName) {
    breadcrumbItems.push({
      name: cityName,
      url: `/indian-buffets/${params['city-state']}`,
    });
  }
  ...
  breadcrumbItems.push({
    name: buffet.name,
    url: `/indian-buffets/${params['city-state']}/${params.slug}`,
  });
  ...
  return (
    <>
      ...
      <ComparisonBundle
        ...
        cuisineType={buffet.cuisineType}
      />

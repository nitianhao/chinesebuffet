...
function BuffetCardSlim({ buffet, citySlug, isChampion }: { buffet: CityBuffetRow; citySlug: string; isChampion?: boolean }) {
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4 hover:shadow-md hover:border-[var(--accent1)] transition-all group">
      <Link
        href={`/indian-buffets/${citySlug}/${buffet.slug}`}
...
export async function generateMetadata({ params }: CityPageProps): Promise<Metadata> {
  ...
  if (timedOut) {
    return {
      title: 'Indian Buffets by City',
      description: 'Find Indian buffets by city. Compare hours, prices, and ratings.',
      alternates: {
        canonical: `${BASE_URL}/indian-buffets/${citySlug}`,
      },
  ...
  if (!data) {
    return {
      ...
      alternates: { canonical: `${BASE_URL}/indian-buffets/${citySlug}` },
    };
  }
  ...
  if (data.buffets.length === 0) {
    return {
      title: `Indian Buffets in ${data.cityName || citySlug}`,
      description: `No Indian buffets currently listed in ${data.cityName || citySlug}.`,
      robots: { index: false, follow: true },
      alternates: {
        canonical: `${BASE_URL}/indian-buffets/${citySlug}`,
      },
  ...
  return {
    title: `${count} Best Indian Buffets in ${cityName} (All-You-Can-Eat ${year})`,
    description: `${count} best all-you-can-eat Indian buffets in ${cityName}, ${state}. Compare ratings, hours & prices. Updated ${year}.`,
    alternates: {
      canonical: `${BASE_URL}/indian-buffets/${citySlug}`,
    },
    robots: { index: true, follow: true },
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: state, item: `${BASE_URL}/indian-buffets/states/${stateAbbr.toLowerCase()}` },
      { '@type': 'ListItem', position: 3, name: cityName, item: `${BASE_URL}/indian-buffets/${citySlug}` },
    ],
  };



  const cityPageUrl = `${BASE_URL}/indian-buffets/${citySlug}`;
  const itemListItems = buffets.slice(0, 50);
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    '@id': `${cityPageUrl}#itemlist`,
    name: `Indian Buffets in ${cityName}, ${state}`,
    numberOfItems: itemListItems.length,
    itemListElement: itemListItems.map((b, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      url: `${BASE_URL}/indian-buffets/${citySlug}/${b.slug}`,
      name: b.name,
    })),
  };
  const webPageSchema = {
    ...
    <SiteShell>
      ...
      <div className="space-y-4">
        <header className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
          <nav className="text-sm text-[var(--muted)] mb-4">
            ...
            <Link
              href={`/indian-buffets/states/${stateAbbr.toLowerCase()}`}
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-[var(--text)] mb-2">
            Indian Buffets in {cityName}, {state}
          </h1>
          ...
          <p className="mt-3 text-sm text-[var(--text-secondary)] max-w-2xl">
            ...
            <Link href={`/indian-buffets/${citySlug}/neighborhoods`} className="text-[var(--accent1)] hover:underline">
        ...
        {buffetCount >= 5 && (
          ...
          >
            ...
            <Link href={`/indian-buffets/${citySlug}/best`} className="inline-flex rounded-full border border-[var(--border)] bg-[var(--surface2)] px-3 py-1.5 text-xs font-medium text-[var(--text)] hover:border-[var(--accent1)] hover:text-[var(--accent1)] transition-colors mr-2 mb-2">Best</Link>
            <Link href={`/indian-buffets/${citySlug}/top-rated`} className="inline-flex rounded-full border border-[var(--border)] bg-[var(--surface2)] px-3 py-1.5 text-xs font-medium text-[var(--text)] hover:border-[var(--accent1)] hover:text-[var(--accent1)] transition-colors mr-2 mb-2">Top Rated</Link>
            <Link href={`/indian-buffets/${citySlug}/cheap`} className="inline-flex rounded-full border border-[var(--border)] bg-[var(--surface2)] px-3 py-1.5 text-xs font-medium text-[var(--text)] hover:border-[var(--accent1)] hover:text-[var(--accent1)] transition-colors mr-2 mb-2">Cheap</Link>
            <Link href={`/indian-buffets/${citySlug}/open-now`} className="inline-flex rounded-full border border-[var(--border)] bg-[var(--surface2)] px-3 py-1.5 text-xs font-medium text-[var(--text)] hover:border-[var(--accent1)] hover:text-[var(--accent1)] transition-colors mr-2 mb-2">Open Now</Link>
          </nav>
        )}
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        ...
        <p className="text-[var(--text-secondary)] leading-relaxed">
          {buffetCount > 2 ? (
            <>
              {cityName} has {buffetCount} Indian {buffetCount === 1 ? 'buffet' : 'buffets'} listed.
              {topBuffets.length > 0 && topBuffets[0].rating && topBuffets[0].reviewsCount && (
                <> The top-rated option is <Link href={`/indian-buffets/${citySlug}/${topBuffets[0].slug}`} className="text-[var(--accent1)] hover:underline">{topBuffets[0].name}</Link>, satisfying diners with a {topBuffets[0].rating.toFixed(1)}★ rating from {topBuffets[0].reviewsCount} reviews.</>
              )}
            ...
            <>
              ...
              {topBuffets.length > 0 && topBuffets[0].rating && topBuffets[0].reviewsCount && (
                <> The main option is <Link href={`/indian-buffets/${citySlug}/${topBuffets[0].slug}`} className="text-[var(--accent1)] hover:underline">{topBuffets[0].name}</Link>, rated {topBuffets[0].rating.toFixed(1)}★ ({topBuffets[0].reviewsCount} reviews).</>
              )}
              {nearbyCities.length > 0 && (
                <> For more choices, check nearby cities like {nearbyCities.slice(0, 3).map((nc, idx, arr) => (
                  <span key={nc.citySlug}>
                    <Link href={`/indian-buffets/${nc.citySlug}`} className="text-[var(--accent1)] hover:underline">{nc.cityName}</Link>
                    {idx < arr.length - 1 ? ', ' : '.'}
                  </span>
                ))}</>
              )}
      ...
      {neighborhoods.length > 0 && (
        <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {neighborhoods.slice(0, 6).map((neighborhood) => (
              <Link
                ...
                href={`/indian-buffets/${citySlug}/neighborhoods/${neighborhood.slug}`}
          ...
          {neighborhoods.length > 6 && (
            <div className="mt-3">
              <Link
                href={`/indian-buffets/${citySlug}/neighborhoods`}
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-[var(--text)]">
            All Indian Buffets in {cityName}
          </h2>
        ...
        <div className="space-y-3">
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)] open:bg-[var(--surface)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
              How many Indian buffets are in {cityName}?
            </summary>
            <p className="px-4 pb-4 text-sm text-[var(--text-secondary)]">
              We found {buffetCount} Indian {buffetCount === 1 ? 'buffet' : 'buffets'} in {cityName}.
            </p>
          </details>
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)] open:bg-[var(--surface)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
              What’s the highest-rated Indian buffet in {cityName}?
            </summary>
            <p className="px-4 pb-4 text-sm text-[var(--text-secondary)]">
              {topBuffets.length > 0 ? (
        ...
        <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
          ...
          <ul className="flex flex-wrap gap-x-6 gap-y-1">
            {otherCitiesInState.map((c) => (
              <li key={c.citySlug}>
                <Link href={`/indian-buffets/${c.citySlug}`} className="text-sm text-[var(--accent1)] hover:underline">
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        ...
        <Link href="/indian-buffets/cities" className="text-[var(--accent1)] hover:opacity-80 font-medium">
          Browse All Cities →
        </Link>
      </section>
    </SiteShell>

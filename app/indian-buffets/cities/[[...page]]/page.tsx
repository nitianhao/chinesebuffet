...
import { getCitiesRollup, CityRollupRow } from '@/lib/indian-rollups';
...
// Pagination is path-based: /indian-buffets/cities (page 1),
//                            /indian-buffets/cities/2 (page 2), etc.
// ---------------------------------------------------------------------------
...
function pageUrl(page: number) {
  return page === 1
    ? `${BASE_URL}/indian-buffets/cities`
    : `${BASE_URL}/indian-buffets/cities/${page}`;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Indian Buffets by City${suffix} - Complete US Directory`,
    description:
      page === 1
        ? `Browse Indian buffets in ${cities.length} cities across the US. Find hours, prices, ratings, and reviews.`
        : `Page ${page} of ${totalPages} — more cities with Indian buffets across the US.`,
...
function PaginationNav({
  ...
  const href = (p: number) =>
    p === 1 ? '/indian-buffets/cities' : `/indian-buffets/cities/${p}`;
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      {page > 1 && (
        <link
          rel="prev"
          href={page === 2 ? '/indian-buffets/cities' : `/indian-buffets/cities/${page - 1}`}
        />
      )}
      {page < totalPages && (
        <link rel="next" href={`/indian-buffets/cities/${page + 1}`} />
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <nav className="text-sm text-[var(--muted)] mb-3">
            ...
            {isPage1 ? (
              ...
              <>
                <Link href="/indian-buffets/cities" className="hover:text-[var(--accent1)]">Cities</Link>
                <span className="mx-2">/</span>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-[var(--text)] mb-1">
            Indian Buffets by City{!isPage1 && ` — Page ${page}`}
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <section>
          ...
          <CityPageSearch cities={searchData} cuisineType="indian">
              ...
              {stateGroups.map((g) => (
                <div key={g.stateAbbr} id={`state-${g.stateAbbr.toLowerCase()}`}>
                  <h3 className="text-base font-semibold text-[var(--text)] mb-2">
                    <Link
                      href={`/indian-buffets/states/${g.stateAbbr.toLowerCase()}`}

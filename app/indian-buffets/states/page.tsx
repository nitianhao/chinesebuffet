...
import { getStatesRollup, STATE_ABBR_TO_NAME } from '@/lib/indian-rollups';
...
export const metadata: Metadata = {
  ...
  description: 'Browse Indian buffets in every US state. Find all-you-can-eat Indian restaurants near you with hours, prices, ratings, and reviews.',
  alternates: {
    canonical: getCanonicalUrl('/indian-buffets/states'),
  },
      ...
      <section className="bg-[var(--surface)] py-8 border-b border-[var(--border)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-[var(--text-secondary)] max-w-3xl">
            Find Indian buffets in any US state. Our directory covers {totalBuffets.toLocaleString()} all-you-can-eat
            Indian restaurants in {totalCities.toLocaleString()} cities. Select a state below to browse local listings
          ...
          {isEmpty ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {states.map((state) => (
                <Link
                  key={state.stateAbbr}
                  href={`/indian-buffets/states/${state.stateAbbr.toLowerCase()}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <ul className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {VALID_REGIONS.map((region) => (
              <li key={region}>
                <Link
                  href={`/indian-buffets/regions/${region}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-white/60">
              Indian Buffets Directory - Find all-you-can-eat Indian buffets by state
            </p>
          </div>
        </div>
      </footer>

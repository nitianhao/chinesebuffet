...
import { getStateCitiesRollup, getCityBuffetsRollup, STATE_ABBR_TO_NAME } from '@/lib/indian-rollups';
...
export async function generateMetadata({ params }: StatePageProps): Promise<Metadata> {
  ...
  const statePath = `/indian-buffets/states/${params.state.toLowerCase()}`;
  if (!stateName) {
    return {
  ...
  if (timedOut) {
    return {
      title: `Indian Buffets in ${stateName}`,
      description: `Find Indian buffets across ${stateName}.`,
      alternates: {
        canonical: `${BASE_URL}/indian-buffets/states/${params.state.toLowerCase()}`,
      },
  ...
  if (!data || data.cities.length === 0) {
    return {
      title: `Indian Buffets in ${stateName}`,
      description: `Find Indian buffets in ${stateName}.`,
      robots: { index: false, follow: true },
      alternates: { canonical: `${BASE_URL}${statePath}` },
    };
  }
  ...
  return {
    title: `Indian Buffets in ${stateName} - ${data.buffetCount} Locations`,
    description: `Find ${data.buffetCount} Indian buffets across ${data.cityCount} cities in ${stateName}. Compare hours, prices, ratings, and locations.`,
    alternates: {
...
export default async function StatePage({ params }: StatePageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'States', item: `${BASE_URL}/indian-buffets/states` },
      {
        '@type': 'ListItem',
        position: 3,
        name: stateName,
        item: `${BASE_URL}/indian-buffets/states/${stateAbbr.toLowerCase()}`,
      },
    ],
  };

  const statePageUrl = `${BASE_URL}/indian-buffets/states/${stateAbbr.toLowerCase()}`;
  const itemListItems = cities.slice(0, 50);
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    '@id': `${statePageUrl}#itemlist`,
    name: `Cities with Indian Buffets in ${stateName}`,
    numberOfItems: itemListItems.length,
    itemListElement: itemListItems.map((c, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      url: `${BASE_URL}/indian-buffets/${c.citySlug}`,
      name: c.cityName,
    })),
  };
  const webPageSchema = {
    ...
    '@type': 'WebPage',
    url: statePageUrl,
    name: `Indian Buffets in ${stateName}`,
  ...
  return (
    <SiteShell>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
          <div>
            ...
            <div className="text-[var(--muted)] mt-2">Indian Buffets</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-[var(--accent1)]">
          ...
          <ul className="flex flex-wrap gap-x-6 gap-y-1">
            {topBuffetsInState.map((b) => (
              <li key={`${b.citySlug}-${b.slug}`}>
                <Link href={`/indian-buffets/${b.citySlug}/${b.slug}`} className="text-sm text-[var(--accent1)] hover:underline">
                  {b.name}
                </Link>
              </li>
            ))}
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="prose max-w-none">
          <p className="text-[var(--text-secondary)] mb-4">
            Discover the best Indian buffets across {stateName}. With {buffetCount.toLocaleString()} locations
            spanning {cityCount} cities, you're sure to find an all-you-can-eat Indian buffet near you.
          </p>
          <p className="text-[var(--text-secondary)] mb-4">
            Whether you're looking for traditional Indian cuisine, Mongolian grill, sushi, or American-Indian
            favorites, our directory covers all the Indian buffet restaurants in {stateName}.
          </p>
          <p className="text-[var(--text-secondary)]">
            Each listing includes hours, prices, ratings, reviews, and detailed location information to help you
            find the perfect Indian buffet experience.
          </p>
        </div>
      </section>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <div className="flex items-center justify-between gap-4 mb-4">
          <h2 className="text-2xl font-bold text-[var(--text)]">
            Cities with Indian Buffets in {stateName}
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {cities.map((city) => (
            <Link
              key={city.citySlug}
              href={`/indian-buffets/${city.citySlug}`}
        ...
        <div className="space-y-3">
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
              How many Indian buffets are in {stateName}?
            </summary>
            <p className="px-4 pb-4 text-sm text-[var(--text-secondary)]">
              We list {buffetCount.toLocaleString()} Indian buffets across {cityCount.toLocaleString()} cities.
            </p>
          </details>
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
            ...
            <p className="px-4 pb-4 text-sm text-[var(--text-secondary)]">
              The top cities section ranks the largest Indian buffet hubs in {stateName}.
            </p>
          </details>
          <details className="rounded-md border border-[var(--border)] bg-[var(--surface2)]">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-[var(--text)]">
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <Link href="/indian-buffets/states" className="text-[var(--accent1)] hover:opacity-80 font-medium">
          ← Back to All States
        </Link>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <p className="text-center text-sm text-[var(--muted)]">
          Indian Buffets Directory - Find all-you-can-eat Indian buffets in {stateName}
        </p>
      </section>
    </SiteShell>
  );

...
import { getCitiesRollup } from '@/lib/indian-rollups';
...
const getCitiesCached = unstable_cache(
  ...
  ['indian-cities-rollup']
);

export const metadata: Metadata = {
  title: 'Indian Buffets by Neighborhood - Browse Local Areas',
  description: 'Browse Indian buffets by neighborhood. Find all-you-can-eat Indian restaurants in local areas across US cities with hours, prices, and reviews.',
  alternates: {
    canonical: getCanonicalUrl('/indian-buffets/neighborhoods'),
  },
...
function CityNeighborhoodCard({ city }: { city: { slug: string; city: string; state: string; buffetCount: number } }) {
  return (
    <Link
      href={`/indian-buffets/${city.slug}/neighborhoods`}
...
export default async function NeighborhoodsIndexPage() {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Neighborhoods', item: `${BASE_URL}/indian-buffets/neighborhoods` },
    ],
  };
    ...
    <SiteShell>
      ...
      <header className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-[var(--text)] mb-2">
          Indian Buffets by Neighborhood
        </h1>
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <p className="text-[var(--text-secondary)] max-w-3xl">
          Find Indian buffets in specific neighborhoods across the United States.
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        ...
        {citiesWithNeighborhoods.length === 0 ? (
          <div className="text-center py-8">
            ...
            <Link href="/indian-buffets/cities" className="text-[var(--accent1)] hover:opacity-80 mt-4 inline-block">
              Browse All Cities →
            </Link>
          </div>
        ) : (
      ...
      <section className="rounded-[var(--section-radius)] border border-[var(--border)] bg-[var(--surface)] p-[var(--section-pad)]">
        <p className="text-center text-sm text-[var(--muted)]">
          Indian Buffets Directory - Find all-you-can-eat Indian buffets by neighborhood
        </p>
      </section>
    </SiteShell>
  );

...
import { STATE_ABBR_TO_NAME } from '@/lib/indian-rollups';
...
export async function generateMetadata({ params }: RegionPageProps): Promise<Metadata> {
  ...
  const canonical = `${BASE_URL}/indian-buffets/regions/${region}`;

  return {
    title: `Indian Buffets in the ${label} - All-You-Can-Eat Directory`,
    description: `Find Indian buffets across the ${label} United States. Browse by state for all-you-can-eat Indian buffets.`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-[var(--text)] mb-3">
            Indian Buffets in the {label}
          </h1>
          <p className="text-[var(--muted)] text-lg leading-relaxed max-w-2xl">
            Browse Indian buffets by state in the {label} United States. Each state page lists cities and buffets with hours, ratings, and directions.
          </p>
        </header>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {stateAbbrs.map((abbr) => (
            <Link
              key={abbr}
              href={`/indian-buffets/states/${abbr.toLowerCase()}`}

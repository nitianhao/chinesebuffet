/**
 * Shared logic for curated SEO filter pages under /indian-buffets/[city-state]/<filter>.
 *
...
import { getCityBuffetsRollup, CityBuffetRow } from '@/lib/indian-rollups';
...
const FILTER_CONFIGS: Record<CityFilter, FilterConfig> = {
  best: {
    label: 'Best',
    title: (city, state) =>
      `Best Indian Buffets in ${city}, ${state} — Top Picks`,
    description: (city, state, count) =>
      `The ${count} best Indian buffets in ${city}, ${state}, ranked by rating and review volume.`,
    intro: (city) =>
      `The best Indian buffets in ${city}, ranked by a combination of customer rating and review volume. These are the highest-quality all-you-can-eat spots locals recommend.`,
    apply: (buffets) =>
      buffets
  ...
  'top-rated': {
    label: 'Top Rated',
    title: (city, state) =>
      `Top Rated Indian Buffets in ${city}, ${state}`,
    description: (city, state, count) =>
      `${count} top-rated Indian buffets in ${city}, ${state}. Sorted by highest customer rating.`,
    intro: (city) =>
      `Top-rated Indian buffets in ${city}, sorted by highest customer rating. These buffets consistently earn the best scores from diners.`,
    apply: (buffets) =>
      buffets
  ...
  cheap: {
    label: 'Cheap',
    title: (city, state) =>
      `Cheap Indian Buffets in ${city}, ${state} — Affordable All-You-Can-Eat`,
    description: (city, state, count) =>
      `${count} affordable Indian buffets in ${city}, ${state}. Budget-friendly all-you-can-eat options sorted by price.`,
    intro: (city) =>
      `Affordable Indian buffets in ${city}. These budget-friendly all-you-can-eat restaurants offer the best value without breaking the bank.`,
    apply: (buffets) => {
  ...
  'open-now': {
    label: 'Open Now',
    title: (city, state) =>
      `Indian Buffets Open Now in ${city}, ${state}`,
    description: (city, state, count) =>
      `Find ${count} Indian buffets open in ${city}, ${state}. Check hours and visit today.`,
    intro: (city) =>
      `Indian buffets in ${city} with listed operating hours. Check individual listings for today's hours and plan your visit.`,
    apply: (buffets) =>
...
export async function buildFilterMetadata(
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_URL}/indian-buffets/${citySlug}/${filter}`,
    },
  };
}
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: cityName,
        item: `${BASE_URL}/indian-buffets/${citySlug}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: config.label,
        item: `${BASE_URL}/indian-buffets/${citySlug}/${filter}`,
      },
    ],
  };
      ...
      >
        <ul className="flex flex-wrap gap-2">
          {CITY_FILTERS.map((f) => (
            <li key={f}>
              <Link
                href={`/indian-buffets/${citySlug}/${f}`}
        ...
        <ul className="divide-y divide-[var(--border)]">
          {filtered.map((b, i) => (
            <li key={b.id} className="py-4 first:pt-0 last:pb-0">
              <Link
                href={`/indian-buffets/${citySlug}/${b.slug}`}
